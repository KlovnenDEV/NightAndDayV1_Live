...
Citizen.CreateThread(function()
	if Config.Framework == 'ESX' then
		while ESX == nil do TriggerEvent('tac:getSharedObject', function(obj) ESX = obj end) Wait(0) end
	...
	elseif Config.Framework == 'DenaliFW' then
		DenaliFW.Functions.TriggerCallback('av_houserobbery:entrada', function(x,y,z,a)
			coordsx, coordsy, coordsz, t = x,y,z,a
			coord = true
		end)
...
AddEventHandler('av_houserobbery:iniciar', function()
	...
	elseif Config.Framework == 'DenaliFW' then
		DenaliFW.Functions.TriggerCallback('av_houserobbery:trabajos', function(puedeRobar)	
			if puedeRobar then
				if not trabajando then
					limpiar()
					trabajando = true
			...
			if IsControlJustReleased(0,74) and distancia < 3 then
				if cerradura then
					if Config.Lockpick then	
						...
						elseif Config.Framework == 'DenaliFW' then
							DenaliFW.Functions.TriggerCallback('av_houserobbery:lockpick',function(tieneLockpick)
								if tieneLockpick then
									ganzuar()
								else
									TriggerEvent('av_houserobbery:notification',Config.Lang['lockpick'])
