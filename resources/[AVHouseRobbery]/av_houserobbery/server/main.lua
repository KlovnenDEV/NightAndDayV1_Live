...
Citizen.CreateThread(function()
	...
	elseif Config.Framework == 'DenaliFW' then
		DenaliFW.Functions.CreateCallback('av_houserobbery:entrada', function(source, cb)		
			cb(x,y,z,num)
		end)
		DenaliFW.Functions.CreateCallback('av_houserobbery:lockpick', function(source,cb)
		...
		DenaliFW.Functions.CreateCallback('av_houserobbery:trabajos', function(source,cb)
			local player = DenaliFW.Functions.GetPlayer(source)
			local identifier = DenaliFW.Functions.GetIdentifier(player, 'license')
			local status = CheckCooldownTimer(identifier)
			if status then
				cb(false)
			else
		...
		print('^2[AV House Robbery]:^7 DenaliFW Framework')
...
AddEventHandler('av_houserobbery:item', function(tipo,m)
	...
	elseif Config.Framework == 'DenaliFW' then
		...
		if tipo == 'random' then
			...
			for i = 1, #items do
				if items[i].type == 'item' then
					...
					TriggerClientEvent('inventory:client:ItemBox', source, DenaliFW.Shared.Items[items[i].itemName], "add")
				elseif items[i].type == 'cash' then
		...
		elseif tipo == 'tv' then
			...
			TriggerClientEvent('inventory:client:ItemBox', source, DenaliFW.Shared.Items['tv'], "add")
		elseif tipo == 'telescopio' then
			player.Functions.AddItem('telescope', 1)
			TriggerClientEvent('inventory:client:ItemBox', source, DenaliFW.Shared.Items['telescope'], "add")
		elseif tipo == 'arte' then
			player.Functions.AddItem('art', 1)
			TriggerClientEvent('inventory:client:ItemBox', source, DenaliFW.Shared.Items['art'], "add")
		elseif tipo == 'laptop' then
			player.Functions.AddItem('laptop', 1)
			TriggerClientEvent('inventory:client:ItemBox', source, DenaliFW.Shared.Items['laptop'], "add")
		else
			local item = props[tipo].item
