    ...
    if data.plate then
        if props.plate == data.plate then
            if not IsPedOnFoot(ped) and IsPedInVehicle(ped, veh, false) then
                DenaliFW.Functions.TriggerCallback('drx_mechanic:upgradeVehicle', function(valid)
                    if valid then
                        SetVehicleProperties(veh, data)
                        DenaliFW.Functions.Notify(data.plate.. ' ' ..Config.Notifications.Modified, 'inform')
                    end
                end, data)
            end
        else
            DenaliFW.Functions.Notify(Config.Notifications.WrongVehNoOwner, 'inform')
        end
    end
end)
...
AddEventHandler('drx_mechanic:returnDatabaseSelection', function(identifier, firstname, lastname, plate, mods)
    SendNUIMessage({
