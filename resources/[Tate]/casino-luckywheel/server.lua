...
AddEventHandler('luckywheel:getwheel', function()
    ...
    if bankBalance >= MinAmount then
        ...
        return TriggerClientEvent('DenaliFW:Notify', source, "You have enough in the bank to spin", "error")
    end
end)
...
AddEventHandler('luckywheel:give', function(source, price)
	...
	elseif price.type == 'item' then
		...
		TriggerClientEvent('DenaliFW:Notify', source, "Congratulations! You won "..price.count.." "..price.name.."!", 'success')
		TriggerClientEvent('inventory:client:ItemBox', source, DenaliFW.Shared.Items[price.name], "add",price.count )

	elseif price.type == 'money' then
		TriggerClientEvent("chCasinoWall:bigWin", source)

		Player.Functions.AddMoney('bank', tonumber(price.count), 'banking-quick-depo')
		TriggerClientEvent('DenaliFW:Notify', source, "Congratulations! You won $"..price.count, 'success')

	elseif price.type == 'weapon' then
		TriggerClientEvent("chCasinoWall:bigWin", source)

		Player.Functions.AddItem(price.name, 1, slot, {["quality"] = 100})
		TriggerClientEvent('DenaliFW:Notify', source, "Congratulations! You won a Pistol!", 'success')
		TriggerClientEvent('inventory:client:ItemBox', source, DenaliFW.Shared.Items[price.name], "add",1)
	end
