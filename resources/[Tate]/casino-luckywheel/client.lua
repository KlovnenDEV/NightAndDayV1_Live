...
AddEventHandler("luckywheel:client:startWheel", function()
	DenaliFW.Functions.TriggerCallback('DenaliFW:HasItem', function(HasItem)
		if HasItem then
			...
			DenaliFW.Functions.Notify('You dont have the proper membership to spin', 'error', 3500)
		end
	end, 'vip')
end)
...
function DenaliFWRequestAnimDict(animDict, cb)
...
function doRoll(index)
    if not _isRolling then
        ...
        DenaliFWRequestAnimDict(lib, function()
...
AddEventHandler("dojLuckywheel:winCar", function() 
	...
	DenaliFW.Functions.SpawnVehicle('demon', function(veh)
		...
		DenaliFW.Functions.Notify("YOU WON THE SHOW CAR! Dodge Challenger SRT Demon!", "primary", 3500)
	end, coords, true)            
end)
