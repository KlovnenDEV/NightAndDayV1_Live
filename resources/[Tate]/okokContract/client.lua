...
AddEventHandler('okokContract:GetVehicleInfo', function(source_playername, date, description, price, source)
	...
	local closestPlayer, playerDistance = DenaliFW.Functions.GetClosestPlayer()
	local sellerID = source
	target = GetPlayerServerId(closestPlayer)

	if closestPlayer ~= -1 and playerDistance <= 3.0 then
		local vehicle = DenaliFW.Functions.GetClosestVehicle(coords)
		...
		if DoesEntityExist(vehicle) and (vehDistance <= 3) then
			local vehProps = DenaliFW.Functions.GetVehicleProperties(vehicle)
			DenaliFW.Functions.TriggerCallback("okokContract:GetTargetName", function(targetName)
