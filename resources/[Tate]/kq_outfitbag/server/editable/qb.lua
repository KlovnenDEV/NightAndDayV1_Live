if Config.qbSettings.enabled then
    ...
    DenaliFW.Functions.CreateUseableItem(Config.bagItem, function(source)
        TriggerClientEvent('kq_outfitbag:placeBag', source, Config.bagItem)
    end)

    for k, item in pairs(Config.additionalItems) do
        DenaliFW.Functions.CreateUseableItem(item, function(source)
    ...
    function RemoveBagItem(player, item)
        ...
        local xPlayer = DenaliFW.Functions.GetPlayer(tonumber(player))
