...
RegisterCommand("kosetut", function(source, args)
    ...
    if not IsPedInAnyVehicle(playerPed) and not IsPedInAnyBoat(playerPed) and not IsPedInAnyHeli(playerPed) and not IsPedInAnyPlane(playerPed) then
        if not holding then
            if args[1] == nil then
                DenaliFW.Functions.Notify("quantity cannot be empty")
            elseif tonumber(args[1]) < 1 then
                DenaliFW.Functions.Notify("you cannot enter negative numbers")
            elseif tonumber(args[1]) > 5 then
                DenaliFW.Functions.Notify("you can only sell 5 pairs at a time")
            else
                holding = true
                itemCount = args[1]
            ...
            DenaliFW.Functions.Notify("you are already cornering now")
        end
    else
        DenaliFW.Functions.Notify("you cannot hold a corner in any vehicle")
    end
end)
    ...
    while true do
        ...
        if holding then
            ...
            if areafound then
                if not npcfound then
                    ...
                    DenaliFW.Functions.Notify("Customer found, moving towards you.")
              ...
              --  print(pedst, npcPED, pedCoord)
                ...
                DenaliFW.Functions.Notify("You can't hold a corner here")
                holding = false
                areafound = false
                areacoord = nil
                npcfound = false
        ...
        if areafound then
            ...
            if pedDistance > radius then
                DenaliFW.Functions.Notify("Corner canceled because you walked away from the zone")
                holding = false
                areafound = false
                areacoord = nil
                npcfound = false
        ...
        if HasEntityBeenDamagedByAnyPed(npcPED) then
            DenaliFW.Functions.Notify("He doesn't want to take anything from you for battering the buyer! You are looking for new customers.")
            newPed = npcPED
    ...
    while true do
		...
		if npcfound then
			if IsControlJustPressed(0, 74) and pedst <= 2 then
				...
				FreezeEntityPosition(newPed, false)
				TaskWanderStandard(newPed, 1, 10)
				DenaliFW.Functions.Notify("You are looking for new customers")
				npcPED = nil
				npcfound = false
				Citizen.Wait(15000)
				DeleteEntity(newPed)
			...
			elseif IsControlJustPressed(0, 38) and pedst <= 2 then
				DenaliFW.Functions.TriggerCallback("kaves_drugs:checkItem", function(output) 
					if output then
						...
						FreezeEntityPosition(newPed, false)
						TaskWanderStandard(newPed, 1, 10)
						DenaliFW.Functions.Notify("The customer did not like you because you did not have the goods...")
						npcPED = nil
						npcfound = false
						Citizen.Wait(15000)
						DeleteEntity(newPed)
