...
AddEventHandler("kaves_drugs:gatheringWeed", function(coord, heading)
    ...
    Citizen.Wait(15000)
    TriggerServerEvent("kaves_drugs:giveItem", "cannabis", Config.Weed.gatheringZone.count)
    Citizen.Wait(1000)
    gathering = false
end)
                ...
                if dst < 2 then
                    ...
                    if IsControlJustPressed(0, 38) then
                        DenaliFW.Functions.TriggerCallback("kaves_drugs:checkItem", function(output)
                            if output then
                                packaged = true
                                TriggerServerEvent("kaves_drugs:removeItem", 'cannabis', Config.Weed.packageZone.count)
                                TriggerEvent("kaves_drugs:packageWeed", v.coord, v.heading, v.rotx, v.roty, v.rotz)
                            else
                                DenaliFW.Functions.Notify("You don't have a weed")
                            end
