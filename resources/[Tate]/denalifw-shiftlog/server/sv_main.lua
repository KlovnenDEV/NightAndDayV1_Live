local DenaliFW = exports['denalifw-core']:GetCoreObject() -- Grabs our core object
...
RegisterServerEvent("denalifw-shiftlog:userjoined")
AddEventHandler("denalifw-shiftlog:userjoined", function(job)
    local id = source
    local xPlayer = DenaliFW.Functions.GetPlayer(id)
...
RegisterServerEvent("denalifw-shiftlog:jobchanged")
AddEventHandler("denalifw-shiftlog:jobchanged", function(old, new, method,src)
    if src == nil then
        src = source
    end
...
RegisterNetEvent('DenaliFW:ToggleDuty', function()
    ...
    if job.name == "police" or job.name == "ambulance" or job.name == "mechanic" then -- job's name here
        if job.onduty == false then
            TriggerEvent("denalifw-shiftlog:jobchanged", currentjob, job, 1, src)
        else
            TriggerEvent("denalifw-shiftlog:jobchanged", currentjob, job, 0, src)  
        end
...
DiscordLog("[denalifw-shiftlog]", "Shift logger started!", 3447003, "police")