...
function initItem(source, item)
    ...
    item.info.hash = tostring(DenaliFW.Shared.RandomInt(2) .. DenaliFW.Shared.RandomStr(3) .. DenaliFW.Shared.RandomInt(1) ..
        DenaliFW.Shared.RandomStr(2) .. DenaliFW.Shared.RandomInt(3) .. DenaliFW.Shared.RandomStr(4))
...
RegisterNetEvent('keep-companion:server:keep-companion:server:compelete_initialization_process_last_step', function(src, item, Player, process_type)
    ...
    if process_type == Config.core_items.groomingkit.item_name then
        ...
        if Player.PlayerData.charinfo.phone ~= petData.info.owner.phone then
            TriggerClientEvent('DenaliFW:Notify', src, Lang:t('error.not_owner_of_pet'), 'error', 2500)
            return
        end
    ...
    if process_type == Config.core_items.groomingkit.item_name then
        TriggerClientEvent('DenaliFW:Notify', src, Lang:t('success.successful_grooming'), 'success', 2500)
...
function Update:xp(source, current_pet_data)
    ...
    if current_pet_data.info.XP > current_level_max_xp(level) then
        ...
        TriggerClientEvent('DenaliFW:Notify', source, msg)
    end
end
...
function Update:food(petData, process_type)
    ...
    --         }
    --     end
    --     TriggerClientEvent('DenaliFW:Notify', source, "Pet food value increased too: " .. mData.content)
    --     return
    -- end
...
DenaliFW.Functions.CreateCallback('keep-companion:server:collar_change_owenership', function(source, cb, data)
    if type(data.new_owner_cid) == "string" then data.new_owner_cid = tonumber(data.new_owner_cid) end
    local player_owner = DenaliFW.Functions.GetPlayer(source)
    if player_owner == nil then return end
    local player_new_owner = DenaliFW.Functions.GetPlayer(data.new_owner_cid)
...
function PlayersCooldown:cleanOflinePlayers()
    local onlinePlayers = DenaliFW.Functions.GetPlayers()
