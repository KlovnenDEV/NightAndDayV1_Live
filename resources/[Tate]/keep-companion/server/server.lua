...
function Pet:spawnPet(source, model, item)
    ...
    if limit == true then
        TriggerClientEvent('DenaliFW:Notify', source, string.format(Lang:t('error.reached_max_allowed_pet'), maxLimit), 'error', 2500)
        return
    end
...
-- food
DenaliFW.Functions.CreateUseableItem(core_items.food.item_name, function(source, item)
    local Player = DenaliFW.Functions.GetPlayer(source)
    if Player == nil then return end
...
RegisterNetEvent('keep-companion:server:increaseFood', function(item)
    ...
    if Player.Functions.RemoveItem('petfood', 1) ~= true then
        TriggerClientEvent('DenaliFW:Notify', source, 'Failed to remove from your inventory', 'error', 2500)
        return
    end
    local petData = Pet:findbyhash(source, item.info.hash)
    petData.info.food = petData.info.food + 50
    TriggerClientEvent('DenaliFW:Notify', source, 'Feeding was successful wait little bit to take effect!', 'success', 2500)
end)

-- change owenership
DenaliFW.Functions.CreateUseableItem(core_items.collar.item_name, function(source, item)
    TriggerClientEvent('keep-companion:client:collar_process', source)
end)

-- rename - name tag
DenaliFW.Functions.CreateUseableItem(core_items.nametag.item_name, function(source, item)
    local Player = DenaliFW.Functions.GetPlayer(source)
    if Player == nil or item == nil then return end
...
RegisterNetEvent('keep-companion:server:rename_name_tag', function(name)
    ...
    if Player.Functions.RemoveItem("petnametag", 1) ~= true then
        TriggerClientEvent('DenaliFW:Notify', source, Lang:t('error.failed_to_remove_item_from_inventory'), 'error', 2500)
        return
    end
...
-- first aid - revive
DenaliFW.Functions.CreateUseableItem(core_items.firstaid.item_name, function(source, item)
    local Player = DenaliFW.Functions.GetPlayer(source)
    if Player == nil then return end

end)

DenaliFW.Functions.CreateUseableItem(core_items.groomingkit.item_name, function(source, item)
    local Player = DenaliFW.Functions.GetPlayer(source)
    if Player == nil then return end
...
local function fillwater_bottle(source, item)
    ...
    if type(item.info) == "table" then
        if item.info.liter > max_c then
            TriggerClientEvent('DenaliFW:Notify', source, 'could not do that already reached max capacity', 'error', 2500)
            return
        elseif item.info.liter == max_c then
            amount = max_c

            TriggerClientEvent('DenaliFW:Notify', source, 'filling already filled bottle has no effect on capacity', 'error', 2500)
        else
        ...
        TriggerClientEvent('DenaliFW:Notify', source, 'Filled bottle', 'success', 2500)
    else
        TriggerClientEvent('DenaliFW:Notify', source, 'Washing new water bottle!', 'primary', 2500)
    end
    save_info_waterbottle(Player, item, amount)
end

DenaliFW.Functions.CreateUseableItem(core_items.waterbottle.item_name, function(source, item)
    local Player = DenaliFW.Functions.GetPlayer(source)
    local water_bottle_refill_value = Config.core_items.waterbottle.settings.water_bottle_refill_value
    if Player == nil then return end
    if Player.Functions.RemoveItem('water_bottle', water_bottle_refill_value) ~= true then
        ...
        TriggerClientEvent('DenaliFW:Notify', source, Lang:t('error.failed_to_remove_item_from_inventory'), 'error', 2500)
        TriggerClientEvent('DenaliFW:Notify', source, msg, 'error', 2500)
        return
    end
...
DenaliFW.Functions.CreateCallback('keep-companion:server:decrease_thirst', function(source, cb, data)
    local player = DenaliFW.Functions.GetPlayer(source)
    ...
    if pet_water_bottle.info == nil then
        TriggerClientEvent('DenaliFW:Notify', source, 'You should wash water bottle first!', 'error', 2500)
        return
    end
    ...
    if pet_water_bottle.info.liter < 0 then
        TriggerClientEvent('DenaliFW:Notify', source, Lang:t('error.not_enough_water_in_your_bottle'), 'error', 2500)
        return
    end
    ...
    TriggerClientEvent('DenaliFW:Notify', source, Lang:t('success.successful_drinking'), 'success', 2500)
...
RegisterNetEvent('keep-companion:server:revivePet', function(item, process_type)
    ...
    if not petData then
        TriggerClientEvent('DenaliFW:Notify', src, Lang:t('error.failed_to_start_procces') .. process_type, 'primary', 2500)
        return
    end
    ...
    if petData.info.health >= pet_maxHealth then
        ...
        TriggerClientEvent('DenaliFW:Notify', src, Lang:t('info.full_life_pet'), 'primary', 2500)
        return
    end

    if not Player.Functions.RemoveItem("firstaidforpet", 1) then
        TriggerClientEvent('DenaliFW:Notify', src, 'Failed to remove from your inventory', 'error', 2500)
        return
    end
    ...
    if process_type and process_type == 'Heal' then
        ...
        TriggerClientEvent('DenaliFW:Notify', src, msg, 'success', 2500)
        return
    end
...
-- all pets
for key, value in pairs(Config.pets) do
    DenaliFW.Functions.CreateUseableItem(value.name, function(source, item)
        ...
        if type(item.info) ~= "table" or (type(item.info) == "table" and item.info.hash == nil) then
            ...
            TriggerClientEvent('DenaliFW:Notify', source, Lang:t('success.pet_initialization_was_successful'), 'success', 2500)
            return
        end
        ...
        if cooldown > 0 then
            ...
            TriggerClientEvent('DenaliFW:Notify', source, msg, 'primary', 2500)
            return
        end
...
function FindWhereIsItem(Player, item, source)
    if Player.PlayerData.items == nil or next(Player.PlayerData.items) == nil then
        TriggerClientEvent('DenaliFW:Notify', source, "no items in inventory!")
        return false
    end
    ...
    TriggerClientEvent('DenaliFW:Notify', source, "Could not find pet")
    return false
end
...
DenaliFW.Functions.CreateCallback('keep-companion:server:renamePet', function(source, cb, item)
    local player = DenaliFW.Functions.GetPlayer(source)
    local current_pet_data = Pet:findbyhash(source, item.hash)
    ...
    if player == nil or current_pet_data == nil or current_pet_data == false or type(item.name) ~= "string" then
        ...
        TriggerClientEvent('DenaliFW:Notify', source, msg, 'error')
        cb(false)
        return
    end
...
DenaliFW.Functions.CreateCallback('keep-companion:server:updatePedData', function(source, cb, clientRes)
    local player = DenaliFW.Functions.GetPlayer(source)
    if player == nil then
        cb(false)
        return
...
DenaliFW.Commands.Add('addpet', 'add a pet to player inventory (Admin Only)', {}, false, function(source, args)
    local PETname = args[1]
    local src = source
    local Player = DenaliFW.Functions.GetPlayer(src)

    Player.Functions.AddItem(PETname, 1)
    TriggerClientEvent("inventory:client:ItemBox", src, DenaliFW.Shared.Items[PETname], "add")
end, 'admin')

DenaliFW.Commands.Add('addItem', 'add item to player inventory (Admin Only)', {}, false, function(source, item)
    local Player = DenaliFW.Functions.GetPlayer(source)
    Player.Functions.AddItem(item[1], 1)
    TriggerClientEvent("inventory:client:ItemBox", source, DenaliFW.Shared.Items[item[1]], "add")
end, 'admin')

DenaliFW.Commands.Add('renamePet', 'rename pet', { { "name", "new pet name" } }, false, function(source, args)
