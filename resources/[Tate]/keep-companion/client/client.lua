local CoreName = exports['denalifw-core']:GetCoreObject()
...
AddEventHandler('keep-companion:client:callCompanion', function(modelName, hostileTowardPlayer, item)
    ...
    CoreName.Functions.Progressbar("callCompanion", "Calling companion", Config.Settings.callCompanionDuration * 1000,
        ...
        DenaliFW.Functions.TriggerCallback('keep-companion:server:updatePedData', function(result)
            if hostileTowardPlayer == true then
                -- if player is not owner of pet it will attack player
                DenaliFW.Functions.Notify(Lang:t('error.not_owner_of_pet'), 'error', 5000)
            end
            ClearPedTasks(ped)
            ...
            exports['denalifw-target']:AddTargetEntity(ped, {
                options = { {
...
function request_healing_process(ped, item, process_type)
    DenaliFW.Functions.TriggerCallback("DenaliFW:HasItem", function(hasitem)
        if not hasitem then DenaliFW.Functions.Notify(Lang:t('error.not_enough_first_aid'), 'error', 5000) return end
...
RegisterNetEvent('keep-companion:client:forceKill', function(hash, reason)
    ...
    DenaliFW.Functions.Notify(msg, 'error', 5000)
end)
...
RegisterNetEvent('keep-companion:client:start_feeding_animation', function()
    ...
    if current_pet == nil then
        DenaliFW.Functions.Notify(Lang:t('error.no_pet_under_control'), 'error', 5000)
        return
    end

    local c_health = GetEntityHealth(current_pet.entity)
    if c_health <= 100.0 or current_pet.itemData.info.health <= 100.0 then
        DenaliFW.Functions.Notify(Lang:t('error.your_pet_is_dead'), 'error', 5000)
        return
    end
...
function start_drinking_animation()
    ...
    CoreName.Functions.Progressbar("pet_drinking", "drinking", Config.core_items.waterbottle.settings.duration * 1000, false, false, {
        ...
        DenaliFW.Functions.TriggerCallback('keep-companion:server:decrease_thirst', function(result)
...
RegisterNetEvent('keep-companion:client:rename_name_tag', function(item)
    ...
    local name = exports['denalifw-input']:ShowInput({
...
RegisterNetEvent('keep-companion:client:rename_name_tagAction', function(name)
    ...
    if activePed.itemData.info.hash == nil or type(name) ~= "string" then
        DenaliFW.Functions.Notify(Lang:t('error.failed_to_start_procces'), 'error', 5000)
        return
    end

    if type(validation) == "table" and next(validation) ~= nil then
        DenaliFW.Functions.Notify(Lang:t('error.failed_to_validate_name'), 'error', 5000)
        if validation.reason == 'badword' then
            DenaliFW.Functions.Notify(Lang:t('error.badword_inside_pet_name'), 'error', 5000)
            print_table(validation.words)
            return
        elseif validation.reason == 'maxCharacter' then
            DenaliFW.Functions.Notify(Lang:t('error.more_than_one_word_as_name'), 'error', 5000)
            return
        end
        return
    end
    ...
    CoreName.Functions.Progressbar("waitingForName", "waiting for Name",
        ...
        DenaliFW.Functions.TriggerCallback('keep-companion:server:renamePet', function(result)
            if type(result) == "string" then
                DenaliFW.Functions.Notify(Lang:t('success.pet_rename_was_successful') .. result, 'success', 5000)
            end
        end, {
...
RegisterNetEvent('keep-companion:client:collar_process', function()
    ...
    if activePed.itemData.info.hash == nil then
        DenaliFW.Functions.Notify(Lang:t('error.failed_to_find_pet'), 'error', 5000)
        return
    end

    local inputData = exports['denalifw-input']:ShowInput({
        header = "New owner id: ",
        submitText = "Confirm",
        inputs = {
            {
    ...
    if inputData then
        ...
        CoreName.Functions.Progressbar("waitingForOwenership", "waiting for new owner",
            ...
            function()
                ...
                DenaliFW.Functions.TriggerCallback('keep-companion:server:collar_change_owenership', function(result)
                    if result.state == false then
                        DenaliFW.Functions.Notify(result.msg, 'error', 5000)
                        return
                    end
                    DenaliFW.Functions.Notify(result.msg, 'success', 5000)
                end, {
