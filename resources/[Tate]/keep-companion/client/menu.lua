    ...
    [2] = {
        ...
        action = function(plyped, activePed)
            ...
            if activePed.canHunt ~= true then
                DenaliFW.Functions.Notify(Lang:t('menu.action_menu.error.pet_unable_to_hunt'), 'error', 5000)
                return false
            end

            if alreadyHunting.state ~= false then
                DenaliFW.Functions.Notify(Lang:t('menu.action_menu.error.already_hunting_something'), 'error', 5000)
                return
            end
        ...
        if option.type == values.TYPE then
            if values.action(plyped, activePed) == true then
                if values.triggerNotification ~= nil then
                    DenaliFW.Functions.Notify(replaceString(values.triggerNotification[1]), 'success', 1500)
                end
            else
                if values.triggerNotification ~= nil then
                    DenaliFW.Functions.Notify(replaceString(values.triggerNotification[2]))
                end
            end
        end
    end
...
RegisterNetEvent('keep-companion:client:initialization_process', function(item, pet_information)
    ...
    DenaliFW.Functions.TriggerCallback("DenaliFW:HasItem", function(hasitem)
        if not hasitem then DenaliFW.Functions.Notify('you need grooming kit', 'error', 5000) return end
        TriggerEvent('keep-companion:client:openMenu_customization', {
            item = item, pet_information = pet_information
        })
