...
function CreateLog(Type, Log, Data)
    DenaliFW.Functions.TriggerCallback('mc-admin/server/create-log', function(result)
    end, Type, Log, Data)
end
...
function IsPlayerAdmin()
    ...
    if Group ~= nil then
        for Rank, Bool in pairs(Group) do
            if Rank == 'admin' and Bool or Rank == 'god' and Bool then
                Retval = true
            end
        end
    end
...
function GetPlayersInArea(Coords, Radius)
	...
	DenaliFW.Functions.TriggerCallback('mc-admin/server/get-active-players-in-radius', function(Players)
...
function GetBans()
    local Prom = promise:new()
    DenaliFW.Functions.TriggerCallback("mc-admin/server/get-bans", function(Bans)
        Prom:resolve(Bans)
    end)
    return Citizen.Await(Prom)
end
...
function GetPlayers()
    local Prom = promise:new()
    DenaliFW.Functions.TriggerCallback("mc-admin/server/get-players", function(Players)
        Prom:resolve(Players)
    end)
    return Citizen.Await(Prom)
end
...
function GetLogs()
    local Prom = promise:new()
    DenaliFW.Functions.TriggerCallback("mc-admin/server/get-logs", function(Logs)
        Prom:resolve(Logs)
    end)
    return Citizen.Await(Prom)
end
