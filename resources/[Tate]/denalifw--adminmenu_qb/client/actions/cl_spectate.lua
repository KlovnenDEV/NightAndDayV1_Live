...
function toggleSpectate(targetPed, targetPlayerId)
    ...
    if isSpectateEnabled then
        ...
        clearGamerTagInfo()
        DoScreenFadeIn(500)
        DenaliFW.Functions.Notify(Lang:t('spectate.stopped', {player = GetPlayerName(storedTargetPlayerId)..' ('..GetPlayerServerId(storedTargetPlayerId)..')'}))
        storedTargetPed = nil
    else
        ...
        DoScreenFadeIn(500)
        DenaliFW.Functions.Notify(Lang:t('spectate.started', {player = GetPlayerName(storedTargetPlayerId)..' ('..GetPlayerServerId(storedTargetPlayerId)..')'}))
...
function cleanupFailedResolve()
    ...
    DenaliFW.Functions.Notify(Lang:t('spectate.stopped_not_found'), 'error')
end
...
RegisterNetEvent('Mercy/client/specPlayer', function(targetServerId, coords)
    ...
    if targetPlayerId == PlayerId() then
        return DenaliFW.Functions.Notify(Lang:t('spectate.self'), 'error')
    end
