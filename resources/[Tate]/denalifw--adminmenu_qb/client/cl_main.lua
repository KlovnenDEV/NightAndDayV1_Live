LoggedIn, DenaliFW, Group = false, exports['denalifw-core']:GetCoreObject(), nil
 
RegisterNetEvent('DenaliFW:Client:OnPlayerLoaded', function()
    Citizen.SetTimeout(1250, function()
        RefreshMenu('All')
        DenaliFW.Functions.TriggerCallback('mc-adminmenu/server/get-permission', function(PGroup)
            Group = PGroup
        end)
        exports['denalifw-adminmenu_qb']:CreateLog('Player Logged In', 'Player Logged In')
        LoggedIn = true
    end)
end)
...
RegisterNetEvent('DenaliFW:Client:OnPlayerUnload', function()
    ...
    exports['denalifw-adminmenu_qb']:CreateLog('Player Logged Out', 'Player Logged Out')
    LoggedIn = false
end)
...
RegisterNUICallback('Admin/GetCharData', function(Data, Cb)
    DenaliFW.Functions.TriggerCallback('mc-admin/server/get-player-data', function(PlayerData)
        Cb(PlayerData)
    end, Data.License)
end)
...
RegisterNUICallback("Admin/GetDateDifference", function(Data, Cb)
    DenaliFW.Functions.TriggerCallback('mc-admin/server/get-date-difference', function(FBans, CAmount)
        Cb({
            Bans = FBans, 
            Amount = CAmount,
        })
