...
function GetPlayerFromLicense(License)
    local Retval = nil
    for _, v in pairs(DenaliFW.Functions.GetPlayers()) do
        local TPlayer = DenaliFW.Functions.GetPlayer(v)
...
function AdminCheck(ServerId)
    ...
    local Retval = false
    if Player ~= nil then
        for Rank, Bool in pairs(DenaliFW.Functions.GetPermission(ServerId)) do
