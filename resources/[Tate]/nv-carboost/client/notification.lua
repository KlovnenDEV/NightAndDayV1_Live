...
function AlertBoosting(netid, type)
    ...
    elseif type == "denalifw-dispatch" then
        exports['denalifw-dispatch']:CarBoosting(vehInfo)
    else
        TriggerServerEvent('nv-carboost:notifypolice', netid)
    end
    ...
    CreateThread(function ()
        while DoesEntityExist(car) do
            ...
            if not Hacked then
                ...
                TriggerServerEvent('nv-carboost:notifyboosting', carPos, car)
            end
        end
    end)
end
...
-- From denalifw-dispatch
RegisterNetEvent('nv-carboost:notifyboosting', function (pos, veh)
...
RegisterNetEvent('nv-carboost:notifypolice', function (car)
    local Player = DenaliFW.Functions.GetPlayerData()
    if Player.job.name == 'police' then
        DenaliFW.Functions.Notify(Lang:t('info.boosting'), 'info')
    end
end)
