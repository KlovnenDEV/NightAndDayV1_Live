...
AddEventHandler('boosting:CreateContract' , function(shit)
  ...
  if(shit == nil) then
    ...
    DenaliFW.Functions.TriggerCallback('boosting:GetExpireTime', function(result)
      if result then
        local data = {
          vehicle = dick,
        ...
        if(HasItem('pixellaptop') == true) then
          if Config['General']["UseNotificationsInsteadOfEmails"] then
            TriggerEvent('DenaliFW:Notify', "You have recieved a "..class.. " Boosting...", "success")
...
RegisterNUICallback("transfercontract", function(data)
  DenaliFW.Functions.TriggerCallback('boosting:getusercontracts', function(usercontracts)
    count = 0
...
RegisterNUICallback('dick', function(data)
  if Config['General']["MinPolice"] == 0 then
    ...
    DenaliFW.Functions.TriggerCallback('boosting:server:GetActivity', function(result)
...
RegisterNUICallback('vin', function(data)
  ...
  if(tonumber(BNEBoosting['functions'].GetCurrentBNE().bne) >= tonumber(Config['Utils']["VIN"]["BNEPrice"])) then
    ...
    TriggerEvent('DenaliFW:Notify', "Not Enough BTC.", "error")
    --[[exports['t-notify']:Alert({
      style  =  'success',
      duration  =  8000,
      title  =  'Boosting Manager',
...
AddEventHandler("boosting:DisablerUsed" , function()
  if OnTheDropoffWay then
    ...
    if (Config['Utils']["Contracts"]["DisableTrackingOnDCB"]) and (Class == "D" or Class == "C" or Class == "B") then
      TriggerEvent('DenaliFW:Notify', "No Tracker.", "error")
      --[[exports['t-notify']:Alert({
        style  =  'inform',
        duration  =  8000,
        title  =  'Boosting Manager',
    ...
    if(DisablerTimes < 1) then
      ...
      if(minigame == true) then
        ...
        DisablerTimes = DisablerTimes + 1
        TriggerServerEvent("boosting:SetBlipTime")
        TriggerEvent('DenaliFW:Notify', "Success.", "success")
        --[[exports['t-notify']:Alert({
          style  =  'success',
          duration  =  8000,
          title  =  'Boosting Manager',
      ...
      if(DisablerTimes == 1) then
        ...
        TriggerServerEvent("boosting:removeblip")
        CanUseComputer = true
        TriggerEvent('DenaliFW:Notify', "Tracker Removed, You can now scratch this vehicle.", "success")
           --[[exports['t-notify']:Alert({
              style  =  'success',
              duration  =  8000,
              title  =  'Boosting Manager',
    ...
    TriggerEvent('DenaliFW:Notify', "You are not boosting right now.", "error")
    --[[exports['t-notify']:Alert({
      style  =  'inform',
      duration  =  8000,
      title  =  'Boosting Manager',
...
AddEventHandler("boosting:DisplayUI" , function()
  ...
  if NuiLoaded then      
    ...
    TriggerEvent('DenaliFW:Notify', "Please wait 3 senconds the nui is loading.", "error")
  --[[exports['t-notify']:Alert({
    style  =  'inform',
    duration  =  8000,
    title  =  'Boosting Manager',
...
AddEventHandler("boosting:ContractDone" , function()
  if CompletedTask then
    DenaliFW.Functions.Notify(Config['Utils']["Notifications"]["DropOffMsg"], "success", 3500)
    TriggerEvent('DenaliFW:Notify', "Leave the car here, You will be paid", "success")
    --[[exports['t-notify']:Alert({
      style = 'inform', 
...
function HasItem(item)
  local hasitem = false
  DenaliFW.Functions.TriggerCallback(Config['CoreSettings']['DenaliFW']["HasItem"], function(result)
    hasitem = result
end, item)
Citizen.Wait(500)
return hasitem
...
local function AddVehicleToGarage()
  ...
  TriggerEvent('DenaliFW:Notify', "Vin scratch complete.", "success")
  --[[exports['t-notify']:Alert({
    style  =  'success',
    duration  =  8000,
    title  =  'Boosting Manager',
  ...
  Citizen.Wait(4000) 
  TriggerEvent('DenaliFW:Notify', "Vehicle now belongs to you, Go park it.", "success")
  --[[exports['t-notify']:Alert({
    style  =  'success',
    duration  =  8000,
    title  =  'Boosting Manager',
    ...
    local vehicleProps = DenaliFW.Functions.GetVehicleProperties(Vehicle)	
...
AddEventHandler("boosting:client:UseComputer" , function()
  if CanUseComputer then
    DenaliFW.Functions.Progressbar("boosting_scratch", "Penetrating the network...", 5000, false, true, {
      ...
      disableCombat = true,
        ...
        TriggerEvent('DenaliFW:Notify', "Failed.", "error")
         --[[exports['t-notify']:Alert({
      style  =  'error',
      duration  =  4000,
      title  =  'Boosting Manager',
    ...
    TriggerEvent('DenaliFW:Notify', "ou cannot use this.", "error")
    --[[exports['t-notify']:Alert({
      style  =  'error',
      duration  =  4000,
      title  =  'Boosting Manager',
...
AddEventHandler("boosting:client:2ndUseComputer" , function()
  DenaliFW.Functions.Progressbar("boosting_scratch", "Wiping online paperwork...", 5000, false, true, {
    disableMovement = true,
    disableCarMovement = true,
    disableMouse = false,
    disableCombat = true,
      ...
      TriggerEvent('DenaliFW:Notify', "Go to the vehicle and scratch the VIN.", "success")
      --[[exports['t-notify']:Alert({
        style  =  'success',
        duration  =  12000,
        title  =  'Boosting Manager',
...
AddEventHandler("boosting:client:ScratchVehicle" , function()
  DenaliFW.Functions.Progressbar("boosting_scratch", "Scratching Vin", 10000, false, true, {
    disableMovement = true,
    disableCarMovement = true,
    disableMouse = false,
    disableCombat = true,
    ...
    if CanUseComputer then
      ...
      if (#(pos - vector3(Config['Utils']["VIN"]["VinLocations"].x, Config['Utils']["VIN"]["VinLocations"].y, Config['Utils']["VIN"]["VinLocations"].z)) < 10.0) then
        ...
        if (#(pos - vector3(Config['Utils']["VIN"]["VinLocations"].x, Config['Utils']["VIN"]["VinLocations"].y, Config['Utils']["VIN"]["VinLocations"].z)) < 1.5) then
          DenaliFW.Functions.DrawText3D(Config['Utils']["VIN"]["VinLocations"].x, Config['Utils']["VIN"]["VinLocations"].y, Config['Utils']["VIN"]["VinLocations"].z, "~g~E~w~ - Use Computer")
