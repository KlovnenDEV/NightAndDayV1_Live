...
DenaliFW.Functions.CreateCallback('boosting:GetExpireTime', function(source, cb)
    local shit = (os.time() + 6 * 3600)
    cb(shit)
end)


DenaliFW.Functions.CreateCallback('boosting:getCurrentBNE', function(source, cb)
    local src = source
...
DenaliFW.Functions.CreateCallback('boosting:removeBNE', function(source, cb , amount)
    local src = source
    local pData = DenaliFW.Functions.GetPlayer(src)
    local cid = pData.PlayerData.citizenid
    if pData ~= nil then
...
DenaliFW.Functions.CreateCallback('boosting:addBne', function(source, cb , amount)
    local src = source
    local pData = DenaliFW.Functions.GetPlayer(src)
    local cid = pData.PlayerData.citizenid
    if pData ~= nil then
...
DenaliFW.Functions.CreateCallback('boosting:server:checkVin', function(source, cb , data)
    local src = source
    local pData = DenaliFW.Functions.GetPlayer(src)
    local cid = pData.PlayerData.citizenid
...
DenaliFW.Functions.CreateCallback('boosting:GetTimeLeft', function(source, cb , data)
        local shit = 2
        cb(shit)
    end)
...
AddEventHandler("boosting:joinQueue", function()
  pSrc = source
  local Player = DenaliFW.Functions.GetPlayer(pSrc)
...
AddEventHandler('boosting:finished' , function()
    ...
    if Config['Utils']["Rewards"]["Type"] == 'item' then
        ...
        TriggerClientEvent("inventory:client:ItemBox", src, DenaliFW.Shared.Items[Config['Utils']["Rewards"]["RewardItemName"]], "add")
...
DenaliFW.Functions.CreateUseableItem("pixellaptop", function(source, item)
    local src = source
...
DenaliFW.Functions.CreateUseableItem("disabler", function(source, item)
...
DenaliFW.Functions.CreateCallback('boosting:server:GetActivity', function(source, cb)
    local PoliceCount = 0
    for k, v in pairs(DenaliFW.Functions.GetPlayers()) do
        local Player = DenaliFW.Functions.GetPlayer(v)
        if Player ~= nil then 
...
DenaliFW.Functions.CreateCallback('boosting:getusercontracts', function(source, cb, target)
