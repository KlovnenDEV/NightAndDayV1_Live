...
AddEventHandler('onResourceStart', function(resourceName)
    if GetCurrentResourceName() == resourceName then
        ...
        DenaliFW.Functions.TriggerCallback('denalifw-scavengerhunt:server:getHunts', function(hunts)
            Hunts = hunts
        end)
    end
end)
                ...
                if scavengerHuntStarted then
                    for k, v in ipairs(Hunts) do
                        if v.completed == false and v.type  == "item" then
                            ...
                            for a, s in pairs(DenaliFW.Functions.GetPlayerData().items) do
                                if s.name == item then
                                    TriggerServerEvent('denalifw-scavengerhunt:server:findGoal', k)
                                end
                            end
                        end
                    end
...
RegisterNetEvent('denalifw-scavengerhunt:client:findGoal')
AddEventHandler('denalifw-scavengerhunt:client:findGoal', function(data)
    if scavengerHuntStarted then
        local goal = data.goal
        TriggerServerEvent('denalifw-scavengerhunt:server:findGoal', goal)
    end
end)

RegisterNetEvent('denalifw-scavengerhunt:client:getFinishGoal')
AddEventHandler('denalifw-scavengerhunt:client:getFinishGoal', function(goal)
    if scavengerHuntStarted then
        if Hunts[goal]["completed"] == false then
            Hunts[goal]["completed"] = true
            DenaliFW.Functions.Notify("Found something!", "success")
        end
        local allDone = true
        ...
        --[[ if allDone then
            Citizen.Wait(2000)
            DenaliFW.Functions.Notify("Scavenger Hunt Completed!", "success")
        end ]]
    end
end)

RegisterNetEvent('denalifw-scavengerhunt:client:registerTeam')
AddEventHandler('denalifw-scavengerhunt:client:registerTeam', function(team)
    selectedTeam = team
    DenaliFW.Functions.Notify("You joined Team "..selectedTeam, "success", 3000)
end)
        ...
        options = {
            {
                ...
                event = "denalifw-scavengerhunt:client:findGoal",
...
function getTeamMembers(teamNumberInc)
    ...
    TriggerServerEvent('denalifw-scavengerhunt:server:getTeamMembers', teamNumberInc)
end

function joinTeam(teamNumberInc)
    teamNumberInc = tonumber(teamNumberInc)
    TriggerServerEvent('denalifw-scavengerhunt:server:joinTeam', teamNumberInc)
end

function leaveTeam(teamNumberInc)
    TriggerServerEvent('denalifw-scavengerhunt:server:leaveTeam', teamNumberInc)
end

function toggleHuntLock(scavengerHuntStartedOutgoing)
    TriggerServerEvent('denalifw-scavengerhunt:server:toggleHuntLock', scavengerHuntStartedOutgoing)
end

RegisterNetEvent('denalifw-scavengerhunt:client:getTeamReceived')
AddEventHandler('denalifw-scavengerhunt:client:getTeamReceived', function(teamIncoming)
    team = teamIncoming
end)
...
function GetTeamResultsCallback(teamResultsTeam)
    TriggerServerEvent('denalifw-scavengerhunt:server:getTeamResults', teamResultsTeam)
end

RegisterNetEvent('denalifw-scavengerhunt:client:RecieveTeamResults')
AddEventHandler('denalifw-scavengerhunt:client:RecieveTeamResults', function(teamResultsIncoming)
    teamResults = teamResultsIncoming
end)

RegisterNetEvent('denalifw-scavengerhunt:client:StartHunt')
AddEventHandler('denalifw-scavengerhunt:client:StartHunt', function(scavengerHuntStartedIncoming)
    scavengerHuntStarted = scavengerHuntStartedIncoming
end)

RegisterNetEvent('denalifw-scavengerhunt:client:joinTeamCommand')
AddEventHandler('denalifw-scavengerhunt:client:joinTeamCommand', function()
    if not scavengerHuntStarted then
        JoinTeam()
    else
        DenaliFW.Functions.Notify("Teams are locked; cannot join.", "error")
    end
end)

RegisterNetEvent('denalifw-scavengerhunt:client:bossCommand')
AddEventHandler('denalifw-scavengerhunt:client:bossCommand', function()
    GetTeamResults()
end)

RegisterNetEvent('denalifw-scavengerhunt:client:openList')
AddEventHandler('denalifw-scavengerhunt:client:openList', function(data)
    if scavengerHuntStarted then
        Anim()
        local clues = {}
        for k, v in ipairs(Hunts) do
