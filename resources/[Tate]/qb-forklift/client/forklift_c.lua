...
RegisterNetEvent('denalifw-forklift:rampanpcdawaj')
AddEventHandler('denalifw-forklift:rampanpcdawaj',function(a)
    Anulowano=false
    if a=='1'then
        if zrespione==true then
            
			DenaliFW.Functions.Notify('Complete the previous order!', 'error')
            return
        end
        ...
        if b==1 then
            ...
            DenaliFW.Functions.Notify('The driver awaits loading the boards.', 'error')
            ZrespPaczuszke('deski')
        elseif b==2 then
            naklejkaAuto=4
            
            DenaliFW.Functions.Notify('The driver is waiting for the fish to be loaded.', 'error')
            ZrespPaczuszke('lody')
        elseif b==3 then
            naklejkaAuto=6
           
            DenaliFW.Functions.Notify('The driver is waiting for the medicine to be loaded.', 'error')
            ZrespPaczuszke('leki')
        elseif b==4 then
            naklejkaAuto=2
          
            DenaliFW.Functions.Notify('The driver awaits loading Logger Light drinks.', 'error')
            ZrespPaczuszke('napoje')
        elseif b==5 then
            naklejkaAuto=1
            
            DenaliFW.Functions.Notify('The driver awaits loading the coffee.', 'error')
            ZrespPaczuszke('kawa')
        end
        ...
        DenaliFW.Functions.Notify('The driver is on his way.', 'success')
        zrespione=true
        silnikOn=true
        Citizen.Wait(900)
        while silnikOn do
        ...
        DenaliFW.Functions.Notify('The driver stopped and opened the trunk', 'error')
        ...
        DenaliFW.Functions.Notify('Package loaded, driver wraps up.', 'error')
        
        DenaliFW.Functions.Notify('You have loaded the package in '..czasikDostaw..' seconds.', 'error')
        if czasikDostaw<41 then
            
            DenaliFW.Functions.Notify('Bonus '..premia1 ..'for fast delivery', 'error')
            TriggerServerEvent("denalifw-forklift:wykonanieMisji",premia1)
            Citizen.Wait(200)
        elseif czasikDostaw>=41 and czasikDostaw<=55 then
            
            DenaliFW.Functions.Notify('Bonus '..premia2 ..' for fast delivery', 'error')
            TriggerServerEvent("denalifw-forklift:wykonanieMisji",premia2)
            Citizen.Wait(200)
        elseif czasikDostaw>=56 and czasikDostaw<=65 then
            
            DenaliFW.Functions.Notify('Bonus '..premia3 ..' for fast delivery', 'error')
            TriggerServerEvent("denalifw-forklift:wykonanieMisji",premia3)
            Citizen.Wait(200)
        elseif czasikDostaw>65 then
           
            DenaliFW.Functions.Notify('No bonus for quick delivery', 'error')
        end
        czasikDostaw=0
        dostawaTimer=false
        TriggerServerEvent("denalifw-forklift:wykonanieMisji",'nie')
        TaskVehicleDriveWander(pilot,transport,50.0,263100)
        Citizen.Wait(15000)
        DeleteEntity(transport)
        DeleteEntity(pilot)zrespione=0
       
        DenaliFW.Functions.Notify('You can accept the next order', 'error')
    elseif a=='2'then
        if zrespione==true then
        ...
        elseif b==3 then
            ...
            DenaliFW.Functions.Notify('The driver is awaiting loading of medications.', 'error')
            ZrespPaczuszke('leki')
        elseif b==4 then
            naklejkaAuto=2
        ...
        DenaliFW.Functions.Notify('The driver is on his way.', 'error')
        zrespione=true
        silnikOn=true
        Citizen.Wait(900)
        while silnikOn do
        ...
        DenaliFW.Functions.Notify('You have loaded the package in '..czasikDostaw..' seconds.', 'success')
        if czasikDostaw<61 then
           
            DenaliFW.Functions.Notify('Bonus '..premia1 ..'$ for fast delivery', 'success')
            TriggerServerEvent("denalifw-forklift:wykonanieMisji",premia1)
            Citizen.Wait(200)
        elseif czasikDostaw>=61 and czasikDostaw<=75 then
           
            DenaliFW.Functions.Notify('Bonus '..premia2 ..'$ for fast delivery', 'success')
            TriggerServerEvent("denalifw-forklift:wykonanieMisji",premia2)
            Citizen.Wait(200)
        elseif
            czasikDostaw>=76 and czasikDostaw<=85 then
           
            DenaliFW.Functions.Notify('Bonus '..premia3 ..'$ for fast delivery', 'success')
            TriggerServerEvent("denalifw-forklift:wykonanieMisji",premia3)
            Citizen.Wait(200)
        elseif
            czasikDostaw>85
...
Citizen.CreateThread(function()
    while true do
		...
		czasikDostaw = czasikDostaw +1
            ...
            DenaliFW.Functions.Notify('The order was canceled because you took too long loading', 'error')
			end
		else
		Citizen.Wait(2000)
		end	
...
RegisterNetEvent('denalifw-forklift:maszHangar')
AddEventHandler('denalifw-forklift:maszHangar', function()
    PrzejetyHangar = 1
end)
        ...
        Citizen.Wait(3)
			...
			if zlecDist <= 1.0 then
				...
				if IsControlJustPressed(0, Keys['E']) then 
					...
					TriggerEvent('denalifw-forklift:rampanpcdawaj','1')
					--TaskStartScenarioInPlace(GetPlayerPed(-1), "WORLD_HUMAN_CLIPBOARD", 0, false)
					Citizen.Wait(2000)
					ClearPedTasks(GetPlayerPed(-1))
                    
                    DenaliFW.Functions.Notify('The package to be delivered is marked on the GPS, use a forklift.', 'success', 5000)
					--else
                   
                    --DenaliFW.Functions.Notify('You must take over the hangar', 'error', 5000)
					--end
				--[[elseif IsControlJustPressed(0, Keys['G']) then
					if PrzejetyHangar == 1 then
                        TriggerServerEvent("denalifw-forklift:przejmijHangar", '1')
                        Citizen.Wait(500)
                    DenaliFW.Functions.Notify('Work started', 'success', 4000)
					else
					TriggerServerEvent("denalifw-forklift:przejmijHangar", '1')
					Citizen.Wait(500)
					end]]
    ...
    while true do
        Citizen.Wait(3)
			...
			if zlecDist2 <= 1.0 then
				...
				if IsControlJustPressed(0, Keys['E']) then 
					TriggerEvent('denalifw-forklift:rampanpcdawaj','2')
					--TaskStartScenarioInPlace(GetPlayerPed(-1), "WORLD_HUMAN_CLIPBOARD", 0, false)
					Citizen.Wait(2000)
					ClearPedTasks(GetPlayerPed(-1))
                    
                    DenaliFW.Functions.Notify('The package to be delivered is marked on the GPS, use a forklift.', 'error')
...
function TakeForklift(a)
    if a=='1'then
        if PojazdWyjety==0 then
            ...
            DenaliFW.Functions.SpawnVehicle(model, function(vehicle)
           ...
           --[[
            ...
            DenaliFW.Functions.Notify('Your previous vehicle has been deleted, you can retreive a new one', 'error')]]--
        end
	...
	while true do 
		...
		if PrzejetyHangar==1 then 
			if b>205.0 then 
				...
				TriggerServerEvent("denalifw-forklift:OdszedlDalekoo")
				Citizen.Wait(1500)
			else 
				Citizen.Wait(3500)
			end
		...
		if dist <= 2.5 then
            ...
            if GetPedInVehicleSeat(vehicled, -1) == GetPlayerPed(-1) then
                ...
                if IsControlJustPressed(0, Keys['E']) then 
                    ...
                    DenaliFW.Functions.DeleteVehicle(veh2)
