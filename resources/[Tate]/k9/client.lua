    ...
    if spawned_ped ~= nil then
        if not following then
            ...
            if has_control then
                ...
                following = true
                attacking = false
                DenaliFW.Functions.Notify(k9_name .. " " .. language.follow, "primary", 5000)
            end
        else
            local has_control = false
            RequestNetworkControl(function(cb)
            ...
            if has_control then
                ...
                following = false
                attacking = false
                DenaliFW.Functions.Notify(k9_name .. " " .. language.stop, "error", 5000)
            end
        end
    end
end)
...
AddEventHandler("K9:ToggleVehicle", function(isRestricted, vehList)
    if not searching then
        if IsPedInAnyVehicle(spawned_ped, false) then
            ...
            DenaliFW.Functions.Notify(k9_name .. " " .. language.exit, "error", 5000)
            Wait(2000)
                ...
                if door ~= false then
                    if isRestricted then
                        if CheckVehicleRestriction(vehicle, vehList) then
                            ...
                            DenaliFW.Functions.Notify(k9_name .. " " .. language.enter, "success", 5000)
                        end
                    else
                        TaskEnterVehicle(spawned_ped, vehicle, -1, door, 2.0, 1, 0)
                        DenaliFW.Functions.Notify(k9_name .. " " .. language.enter, "success", 5000)
...
AddEventHandler("K9:ToggleAttack", function(target)
    if not attacking and not searching then
        ...
        attacking = true
        following = false
        DenaliFW.Functions.Notify(k9_name .. " " .. language.attack, "error", 5000)
    end
end)
    ...
    AddEventHandler("K9:SearchVehicle", function(openDoors)
        ...
        if vehicle ~= 0 and not searching then
            ...
            if carplate ~= nil then
                ...
                DenaliFW.Functions.Notify(k9_name .. " has began searching...", "primary", 5000) -- [text] = message, [type] = primary | error | success, [length] = time till fadeout.
                ...
                DenaliFW.Functions.Notify("There is no vehicle nearby", "error", 5000) -- [text] = message, [type] = primary | error | success, [length] = time till fadeout.
            end
        else
            DenaliFW.Functions.Notify("There is no vehicle nearby", "error", 5000) -- [text] = message, [type] = primary | error | success, [length] = time till fadeout.
        end
...
Citizen.CreateThread(function()
    while true do
        ...
        -- Trigger Opens Menu
        if IsControlJustPressed(1, 161) then
            TriggerServerEvent("K9:RequestOpenMenu")
        end

        -- Trigger Attack
        if IsControlJustPressed(1, 162) and IsPlayerFreeAiming(PlayerId()) then
        ...
        -- Trigger Follow
        if IsControlJustPressed(1, 163) and not IsPlayerFreeAiming(PlayerId()) then
