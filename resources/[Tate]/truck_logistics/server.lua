--[[
 ...
 | |____  | (_) | | | | |  \ V /  |  __/ | |    | |_  |  __/ | (_| |   | |_) | | |_| |      \  /    | | | | \ \  | |_| |  ____) |
  \_____|  \___/  |_| |_|   \_/    \___| |_|     \__|  \___|  \__,_|   |_.__/   \__, |       \/     |_| |_|  \_\  \__,_| |_____/ 
                                                                                 __/ |                                           
                                                                                |___/                                            
-- Converted by ViRuS for DenaliFW Framework - https://github.com/DenaliFW-framework --
]]
...
Citizen.CreateThread(function()
	...
	while true do
		...
		local users = DenaliFW.Functions.GetPlayers()
	...
	while true do 
		local sql = exports['ghmattimysql']:execute("SELECT * FROM `trucker_drivers` ", function(result)
			...
			for k,v in pairs(data) do
				local source = DenaliFW.Functions.GetPlayerByCitizenId(v.user_id)
				if Config.trabalhos.gera_dinheiro_offline or source then
					if tryGetTruckerMoney(v.user_id,v.price + v.price_per_km) then
						...
						if source then
							TriggerClientEvent("DenaliFW:Notify", source, Lang[Config.lang]['driver_failed']:format(v.name), "error")
						end
					end
		...
		local data = exports['ghmattimysql']:execute("SELECT * FROM trucker_loans", function(data)
			for k,v in pairs(data) do
				if v.timer + Config.emprestimos.cooldown < os.time() then
					local source = DenaliFW.Functions.GetPlayerByCitizenId(tonumber(v.user_id))
					if tryGetTruckerMoney(v.user_id,v.day_cost) then
						...
						if source then
							TriggerClientEvent("DenaliFW:Notify",source, Lang[Config.lang]['no_loan_money'], "primary")
						else
												...
												if tonumber(query[1].contract_type) == 0 then
													...
													query_truck = exports['ghmattimysql']:execute("SELECT * FROM `trucker_trucks` WHERE driver = 0 AND user_id = @user_id", {['@user_id'] = user_id}, function(query_truck)
														if query_truck and query_truck[1] then
															...
															TriggerClientEvent("DenaliFW:Notify",source, Lang[Config.lang]['own_truck'], "error")
														end
													end)
												end
											else
												TriggerClientEvent("DenaliFW:Notify",source, Lang[Config.lang]['no_skill_1'], "error")
											end
										else
											TriggerClientEvent("DenaliFW:Notify",source,Lang[Config.lang]['no_skill_2'], "error")
										end
									else
										TriggerClientEvent("DenaliFW:Notify",source,Lang[Config.lang]['no_skill_3'], "error")
									end
								else
									TriggerClientEvent("DenaliFW:Notify",source,Lang[Config.lang]['no_skill_4'], "error")
								end
							else
								TriggerClientEvent("DenaliFW:Notify",source,Lang[Config.lang]['no_skill_5'], "error")
							end
						end
					end)
				else
					TriggerClientEvent("DenaliFW:Notify",source,Lang[Config.lang]['job_already_started'], "error")
				end
			end)
		end
	...
	if user_id then
		local sql = exports['ghmattimysql']:execute("SELECT * FROM `trucker_users` WHERE user_id = @user_id", {['@user_id'] = user_id}, function(result)
			...
			if query.skill_points >= (data.value - query[data.id]) then
				...
				TriggerClientEvent("DenaliFW:Notify", source, Lang[Config.lang]['upgraded_skill'], "success")
				openUI(source,true)
			else
				TriggerClientEvent("DenaliFW:Notify",source, Lang[Config.lang]['insufficient_skill_points'], "error")
			end
		end)
	end
end)
			...
			if query then
				...
				if amount > 0 then
					if tryGetTruckerMoney(user_id,amount) then
						...
						TriggerClientEvent("DenaliFW:Notify",source, Lang[Config.lang]['repaired'], "success")
						openUI(source,true)
					else
						TriggerClientEvent("DenaliFW:Notify",source, Lang[Config.lang]['insufficiente_funds'], "error")
					end
				else
					TriggerClientEvent("DenaliFW:Notify",source, Lang[Config.lang]['not_repaired'], "error")
				end
			else
				TriggerClientEvent("DenaliFW:Notify",source, Lang[Config.lang]['have_no_truck'], "error")
			end
		end)
	end
end)
...
AddEventHandler("truck_logistics:buyTruck",function(data)
	...
	if user_id then
		if tryGetTruckerMoney(user_id,tonumber(data.price)) then
			...
			TriggerClientEvent("DenaliFW:Notify",source,Lang[Config.lang]['bought'], "success")
		...
		if user_id then
			local sql = exports['ghmattimysql']:execute("SELECT * FROM `trucker_trucks` WHERE truck_id = @truck_id", {['@truck_id'] = data.truck_id}, function(result)
				...
				if query then 
					...
					TriggerClientEvent("DenaliFW:Notify",source,Lang[Config.lang]['sold'], "success")
			...
			if count <= Config.motoristas.max_motoristas_por_player then
				local sql = exports['ghmattimysql']:execute("SELECT price FROM trucker_drivers WHERE driver_id = @driver_id", {['@driver_id'] = driver_id}, function(result2)
					...
					if tryGetTruckerMoney(user_id,query.price) then
						...
						TriggerClientEvent("DenaliFW:Notify",source, Lang[Config.lang]['hired'], "success")
						openUI(source,true)
					else
						TriggerClientEvent("DenaliFW:Notify",source,"Dinero insuficiente", "error")
					end
				end)
			else
				TriggerClientEvent("DenaliFW:Notify",source,Lang[Config.lang]['max_drivers'], "error")
			end
		end)
	end
end)
...
AddEventHandler("truck_logistics:fireDriver",function(driver_id)
	...
	if user_id then
		...
		TriggerClientEvent("DenaliFW:Notify",source, Lang[Config.lang]['fired'], "success")
		openUI(source,true)
	end
end)
				...
				if not query or not query.remaining_amount or query.remaining_amount <= 0 then
					local sql = exports['ghmattimysql']:execute("SELECT money FROM `trucker_users` WHERE user_id = @user_id", {['@user_id'] = user_id}, function(sql2)
						...
						if amount and amount > 0 then
							...
							TriggerClientEvent("DenaliFW:Notify",source, Lang[Config.lang]['money_withdrawn'], "success")
							openUI(source,true)
						else
							TriggerClientEvent("DenaliFW:Notify",source,Lang[Config.lang]['insufficiente_money'], "error")
						end
					end)
				else
					TriggerClientEvent("DenaliFW:Notify",source,Lang[Config.lang]['pay_loans'], "error")
				end
			end)
		end
		debug_cooldown[source] = nil
	...
	if user_id then
		...
		if amount and amount > 0 then
			...
			if money >= amount then
				...
				giveTruckerMoney(user_id,amount)
				TriggerClientEvent("DenaliFW:Notify",source, Lang[Config.lang]['money_deposited'], "success")
				openUI(source,true)
			else
				TriggerClientEvent("DenaliFW:Notify",source,Lang[Config.lang]['insufficiente_money'], "error")
			end
		else
			TriggerClientEvent("DenaliFW:Notify",source,Lang[Config.lang]['invalid_value'], "error")
		end
	end
end)
	...
	if debug_cooldown[source] == nil then
		...
		if user_id then
			...
			if amount_loans + Config.emprestimos.valores[data.loan_id][1] <= getMaxEmprestimo(user_id) then
				...
				TriggerClientEvent("DenaliFW:Notify",source, Lang[Config.lang]['loan'], "success")
				openUI(source,true)
			else
				TriggerClientEvent("DenaliFW:Notify",source,Lang[Config.lang]['no_loan'], "error")
			end
		end)
		end
		debug_cooldown[source] = nil
		...
		local sql = exports['ghmattimysql']:execute("SELECT * FROM `trucker_loans` WHERE id = @id",{['@id'] = data.loan_id}, function(result)
			local query = result[1]
				if tryGetTruckerMoney(user_id, query.remaining_amount) then
					...
					TriggerClientEvent("DenaliFW:Notify",source, Lang[Config.lang]['loan_paid'], "success")
					openUI(source,true)
				else
					TriggerClientEvent("DenaliFW:Notify",source,Lang[Config.lang]['insufficiente_funds'], "error")
				end
...
AddEventHandler("truck_logistics:finishJob",function(data,distance,reward,truck_data,truck_engine,truck_body,trailer_body)
	...
	if user_id then
		...
		TriggerClientEvent("DenaliFW:Notify",source, Lang[Config.lang]['reward']:format(tostring(received_amount),tostring(trailer_body*100),tostring(received_xp)), "success")
