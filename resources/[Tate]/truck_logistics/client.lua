...
RegisterNUICallback('startJob', function(data, cb)
	if cooldown == nil then
		...
		if not loading then
			...
			DenaliFW.Functions.Notify(Lang[Config.lang]['loading_trailer'], "primary")
			closeUI()
		end
...
RegisterNUICallback('repairTruck', function(data, cb)
	if truck and not rentTruck then
		DenaliFW.Functions.Notify(Lang[Config.lang]['store_truck'], "error")
		return
	end
...
RegisterNUICallback('sellTruck', function(data, cb)
	if cooldown == nil then
		...
		if truck and not rentTruck then
			DenaliFW.Functions.Notify(Lang[Config.lang]['store_truck_2'], "error")
			return
		end
...
RegisterNUICallback('spawnTruck', function(data, cb)
	if cooldown == nil then
		...
		if not loading then
			...
			DenaliFW.Functions.Notify(Lang[Config.lang]['loading_truck'], "primary")
			closeUI()
		end
...
AddEventHandler('truck_logistics:spawnTruck', function(truck_data)
	...
	if truck then
		DenaliFW.Functions.Notify(Lang[Config.lang]['already_has_truck'], "error")
		return
	end
	...
	while i > 0 do
		...
		if checkPos == false then
			if i <= 1 then
				DenaliFW.Functions.Notify(Lang[Config.lang]['occupied_places'], "error")
				loading = false
				return
			end
		else
	...
	DenaliFW.Functions.Notify(Lang[Config.lang]['already_is_in_garage'], "success")
	loading = false
...
AddEventHandler('truck_logistics:startContract', function(data,job_distance,reward,truck_data)
	...
	if trailer or rentTruck then
		DenaliFW.Functions.Notify(Lang[Config.lang]['already_has_cargo'], "error")
		return
	end
	...
	if truck and data.contract_type == 0 then
		DenaliFW.Functions.Notify(Lang[Config.lang]['must_store_truck'], "error")
		return
	end
	...
	DenaliFW.Functions.Notify(Lang[Config.lang]['started_job'], "success")
	loading = false
	...
	while IsEntityAVehicle(trailer) do 
		...
		if bodyhealth <= 150 then
			PlaySoundFrontend(-1, "PROPERTY_PURCHASE", "HUD_AWARDS", 0)
			DenaliFW.Functions.Notify(Lang[Config.lang]['failed'], "error")
...
function resetLoading()
	...
	if loading == true then
		DenaliFW.Functions.Notify(Lang[Config.lang]['loading_fail'], "error")
		loading = false
	end
end
...
AddEventHandler('truck_logistics:vehicleClientLock', function()
	...
	if DoesEntityExist(v) and IsEntityAVehicle(v) then
		...
		if lock == 1 then
			...
			DenaliFW.Functions.Notify(Lang[Config.lang]['locked'], "primary", 8000)
		else
			SetVehicleDoorsLocked(v,1)
			DenaliFW.Functions.Notify(Lang[Config.lang]['unlocked'], "primary", 8000)
		end
