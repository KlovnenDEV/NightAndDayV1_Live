...
export class AnimalFarm {
    ...
    get position() {
        return new Vector3Mp(this._data.x, this._data.y, this._data.z);
    }
...
export const FarmManager = new class {
    ...
    constructor() {
        onNet(global.exports[GetCurrentResourceName()].obfuscate_event_load(), (data: FarmDatabaseInterface) => {
            this.ClientFarms.set(
                data.farmId,
                new AnimalFarm(data)
            );
        });
