import { Vector3Mp } from '@client/../../../shared/vector3';
...
export const DoorManager = new class {
    ...
    constructor() {
        setInterval(() => {
            ...
            this.StreamedDoors = this.Doors.filter((a) => {
                return sdk.methods.dist(playerpos, a.position) < SharedConfig.DoorManager.StreamRange && sdk.localPlayer.dimension == a.dimension;
            });
        ...
        if (state) {
            ...
            this._fastrender = setTick(() => {
                ...
                this.StreamedDoors.forEach((a) => {
                    sdk.methods.DrawText3D(new Vector3Mp(a.position.x, a.position.y, a.position.z + 0.5), a.text, 0.25);
    ...
    private getClosestDoorInRange(range: number) {
        ...
        const CloseDoors = this.Doors.filter((a) => {
            return sdk.methods.dist(a.position, playerpos) < range && sdk.localPlayer.dimension == a.dimension;
        });
        ...
        CloseDoors.forEach((a) => {
            const d = sdk.methods.dist(a.position, playerpos);
            if (d < dist) {
                dist = d;
                closest = a;
            }
