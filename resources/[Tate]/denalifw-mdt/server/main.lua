...
AddEventHandler("onResourceStart", function(resourceName)
	if (resourceName == 'denalifw-mdt') then
        activeUnits = {}
    end
end)
...
DenaliFW.Functions.CreateCallback('mdt:server:SearchProfile', function(source, cb, sentData)
...
DenaliFW.Functions.CreateCallback("mdt:server:getWarrants", function(source, cb)
    local WarrantData = {}
...
DenaliFW.Functions.CreateCallback('mdt:server:OpenDashboard', function(source, cb)
...
DenaliFW.Functions.CreateCallback('mdt:server:GetProfileData', function(source, cb, sentId)
...
DenaliFW.Functions.CreateCallback('mdt:server:SearchVehicles', function(source, cb, sentData)
	if not sentData then  return cb({}) end
	local src = source
	local PlayerData = GetPlayerData(src)
					...
					if impoundInfo.impoundActive then
						...
						if vehicle.impoundid ~= nil then
							...
							if result then
								...
								result.plate = plate
								TriggerClientEvent('denalifw-mdt:client:TakeOutImpound', src, result)
							end
...
RegisterNetEvent('mdt:server:setCallsign', function(cid, newcallsign)
	local Player = DenaliFW.Functions.GetPlayerByCitizenId(cid)
...
RegisterNetEvent('mdt:server:setWaypoint', function(callid)
	...
	if JobType == 'police' or JobType == 'ambulance' then
		if callid then
			local calls = exports['denalifw-dispatch']:GetDispatchCalls()
...
RegisterNetEvent('mdt:server:setRadio', function(cid, newRadio)
	...
	if Player.PlayerData.citizenid ~= cid then
		TriggerClientEvent("DenaliFW:Notify", src, 'You can only change your radio!', 'error')
		return
	else
		...
		if radio ~= nil then
			...
			TriggerClientEvent("DenaliFW:Notify", src, 'You do not have a radio!', 'error')
		end
	end
