            ...
            if IsPedInAnyVehicle(plyPed, false) then
                ...
                if DoesEntityExist(plyVehicle) and validVehicle(GetEntityModel(plyVehicle)) then
                    ...
                    if IsControlPressed(0, Config.KeyBind) then
                        changeClass()
                        denalifw.Functions.Notify(("Changed class to %s"):format(currentClass))
                        updateHandliong(plyVehicle)
                    end
                    Sleeper = 200
                end
...
RegisterNetEvent('denalifw:Client:OnPlayerLoaded', function()
    player = denalifw.Functions.GetPlayerData()
...
RegisterCommand("carhash", function()
    print(('^6[^3denalifw-pursuitmode^6]^0: Vehicle Hash "%s"'):format(GetEntityModel(GetVehiclePedIsIn(PlayerPedId(), -1))))
end)
