...
function StartTrainHeist()
    DenaliFW.Functions.TriggerCallback('trainheist:server:checkPoliceCount', function(status)
        if status then
            DenaliFW.Functions.TriggerCallback('trainheist:server:checkTime', function(time)
                if time then
                    DenaliFW.Functions.Notify(Strings['goto_ambush'], "success")
...
function OpenContainer(index)
    DenaliFW.Functions.TriggerCallback('trainheist:server:hasItem', function(hasItem)
        if hasItem then
            local ped = PlayerPedId()
            local pedCo = GetEntityCoords(ped)
			...
			DenaliFW.Functions.Notify(Strings['need_this'], "error")
        end
