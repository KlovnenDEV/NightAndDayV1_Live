RegisterServerEvent('qb-overlord-laundering:use', function(key)
	...
	if Player.PlayerData.money.cash >= tonumber(machine.cost) then
		...
		TriggerClientEvent('DenaliFW:Notify', source, "You cannot afford this!", "error")
	end
end)
...
RegisterServerEvent('qb-overlord-laundering:load', function(machine)
	...
	if num_bags < 1 then
		TriggerClientEvent('DenaliFW:Notify', src, 'Missing item...', 'error')
	...
	if CONFIG['PoliceIncrease'] then
		...
		for k, v in pairs(DenaliFW.Functions.GetPlayers()) do
			local job = DenaliFW.Functions.GetPlayer(v).PlayerData.job
	...
	TriggerClientEvent('DenaliFW:Notify', src, 'You loaded '..num_bags..'x '..DenaliFW.Shared.Items['markedbills'].label..' ($'..total_worth..')', 'success')

	Citizen.Wait(timer) -- probably a better way to do this but not too fussed about serverside performance

	TriggerClientEvent('denalifw-phone:client:LaunderNotify', src)
