...
RegisterServerEvent('OD-doorlock:updateState')
AddEventHandler('OD-doorlock:updateState', function(doorID, locked, src, usedLockpick)
	local playerId = source
	local xPlayer = DenaliFW.Functions.GetPlayer(playerId)
	--local PlayerData = DenaliFW.Functions.GetPlayerData()

	if type(doorID) ~= 'number' then
		print(('OD-doorlock: %s (%s) didn\'t send a number! (Sent %s)'):format(xPlayer.PlayerData.name(), xPlayer.PlayerData.steam(), doorID))
		return
	end

	if type(locked) ~= 'boolean' then
		print(('OD-doorlock: %s (%s) attempted to update invalid state! (Sent %s)'):format(xPlayer.PlayerData.name(), xPlayer.PlayerData.steam(), locked))
		return
	end

	if not Config.DoorList[doorID] then
		print(('OD-doorlock: %s (%s) attempted to update invalid door! (Sent %s)'):format(xPlayer.PlayerData.name(), xPlayer.PlayerData.steam(), doorID))
		return
	end
	...
	if not src then TriggerClientEvent('OD-doorlock:setState', -1, playerId, doorID, locked)
	else TriggerClientEvent('OD-doorlock:setState', -1, playerId, doorID, locked, src) end
	...
	if Config.DoorList[doorID].autoLock then
		Citizen.SetTimeout(Config.DoorList[doorID].autoLock, function()
			...
			TriggerClientEvent('OD-doorlock:setState', -1, -1, doorID, true)
		end)
	end
end)

DenaliFW.Functions.CreateCallback('OD-doorlock:getDoorInfo', function(source, cb)
	cb(doorInfo)
end)
...
--	grade[1] = xPlayer.job.grade
	...
	if not canOpen and doorID.authorizedJobs then
		...
		if not canOpen and not doorID.items then
			print(('OD-doorlock: %s (%s) was not authorized to open a locked door!'):format(xPlayer.PlayerData.name, xPlayer.PlayerData.steam))
		end
	end
...
RegisterServerEvent('OD-doorlock:server:updateState')
AddEventHandler('OD-doorlock:server:updateState', function(doorID, state)
	local playerId = source
	local xPlayer = DenaliFW.Functions.GetPlayer(playerId)
	
	TriggerClientEvent('OD-doorlock:setState', -1, playerId, doorID, state)
...
RegisterCommand('newdoor', function(playerId, args, rawCommand)
	TriggerClientEvent('OD-doorlock:newDoorSetup', playerId, args)
end, true)
...
RegisterCommand('testdoors', function(playerId, args)
	...
	if doorlock == "false" then lock = nil 
		print(doorno, lock)
		TriggerEvent('OD-doorlock:server:updateState', playerId, doorno, lock)
...
RegisterServerEvent('OD-doorlock:newDoorCreate')
AddEventHandler('OD-doorlock:newDoorCreate', function(model, heading, coords, jobs, item, doorLocked, maxDistance, slides, garage, doubleDoor, doorname)
	local src = source
	xPlayer = DenaliFW.Functions.GetPlayer(src)
	...
	TriggerClientEvent('OD-doorlock:newDoorAdded', -1, newDoor, doorID, doorLocked)
end)
...
--[[RegisterCommand('testdoors', function(playerId, args, rawCommand)
	for k, v in pairs(doorInfo) do
		...
		doorInfo[k] = lock
		print(doorInfo)
		TriggerClientEvent('OD-doorlock:setState', -1, k, lock)
	end
end, true)]]--
