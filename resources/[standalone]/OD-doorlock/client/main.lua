...
Citizen.CreateThread(function()
	...
	DenaliFW.PlayerData = DenaliFW.Functions.GetPlayerData()
	-- Sync doors with the server
	Citizen.Wait(1000)
	DenaliFW.Functions.TriggerCallback('OD-doorlock:getDoorInfo', function(doorInfo)
...
RegisterNetEvent('OD-doorlock:setState')
AddEventHandler('OD-doorlock:setState', function(sid, doorID, locked, src)
...
AddEventHandler('DenaliFW:Player:SetPlayerData', function(job)
	DenaliFW.PlayerData.job = job
end)

RegisterNetEvent('DenaliFW:Player:SetPlayerData2')
AddEventHandler('DenaliFW:Player:SetPlayerData2', function(job)
	DenaliFW.PlayerData.job2 = job
end)
...
-- `exports.OD-doorlock:updateDoors()`
...
RegisterCommand('doorlock', function()
	if not isDead and not isCuffed and closestDoor and IsAuthorized(closestV) then
		...
		TriggerServerEvent('OD-doorlock:updateState', closestDoor, locked, src) -- Broadcast new state of the door to everyone
	end
end)
...
AddEventHandler('lockpicks:UseLockpick', function()
	...
	DenaliFW.Functions.TriggerCallback('OD-radio:server:GetItem', function(hasItem)
		if not isDead and not isCuffed and closestDoor then
			if closestV.lockpick then
				if hasItem then
					TriggerEvent('OD-lockpick:client:openLockpick', lockpickFinish)
				else
					DenaliFW.Functions.Notify("You are missing a toolkit..", "error")
				end
			else
				DenaliFW.Functions.Notify('The door is too strong', 'error', 2500)
			end
...
function lockpickFinish(success)
    if success then
		DenaliFW.Functions.Notify('Succes!', 'success', 2500)
		setDoorLocking(closestDoor, false)
		print(closestDoor)
    else
        DenaliFW.Functions.Notify('Failed..', 'error', 2500)
    end
end
...
function setDoorLocking(doorId, key)
    ...
    SetTimeout(400, function()
		...
		print(doorId)
		TriggerServerEvent('OD-doorlock:server:updateState',doorId, false)
	end)
end
...
RegisterNetEvent('OD-doorlock:newDoorSetup')
AddEventHandler('OD-doorlock:newDoorSetup', function(args)
	if not args[1] then
	...
	if doorType == 'door' or doorType == 'sliding' or doorType == 'garage' then
		...
		TriggerServerEvent('OD-doorlock:newDoorCreate', model, heading, coords, jobs, item, doorLocked, maxDistance, slides, garage, false, doorname)
	...
	elseif doorType == 'double' or doorType == 'doublesliding' then
		...
		if arg then doorname = arg.doorname end
		TriggerServerEvent('OD-doorlock:newDoorCreate', model, heading, coords, jobs, item, doorLocked, maxDistance, slides, garage, true, doorname)
		print('Successfully sent door data to the server')
		arg = nil
	end
end)
...
RegisterNetEvent('OD-doorlock:newDoorAdded')
AddEventHandler('OD-doorlock:newDoorAdded', function(newDoor, doorID, locked)
	Config.DoorList[doorID] = newDoor
	updateDoors()
	TriggerEvent('OD-doorlock:setState', GetPlayerServerId(PlayerId()), doorID, locked)
end)
