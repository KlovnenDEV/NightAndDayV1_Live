...
KashChosen = function(charId)
  ...
  while not xPlayer do xPlayer = DenaliFW.Functions.GetPlayer(_source); Wait(0); end
           ...
           -- SqlFetch("SELECT TABLE_NAME FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA=@table AND TABLE_NAME=@name",{['@table'] = dbName,['@name']  = (Config and Config.WarehouseTable or "vehicles")},function(r2)  
            ...
            --  if type(r2) == "table" and r2[1] and r2[1].TABLE_NAME == (Config and Config.WarehouseTable or "vehicles") then
                ...
                SqlFetch("SELECT * FROM "..(Config and Config.ShopTable or "vehicle_shops"),{},function(shopData)
                  ...
                  vehData = DenaliFW.Shared.VehicleModels
...
local xplauyerc = DenaliFW.Functions.GetPlayer(VehicleShops[shopId].owner)
local vehiclemodel = DenaliFW.Shared.VehicleModels[props.model].model
...
TryBuy = function(source,callback,shop,veh,plate,class)
  ...
  if can_purchase then
    ...
    local vehiclemodel = DenaliFW.Shared.VehicleModels[plate.model].model
...
local xplayer = DenaliFW.Functions.GetIdentifier("steam",VehicleShops[shop].owner)
...
AddFunds = function(shop_key,amount)
  ...
  if can_purchase then      
    ...
    TriggerClientEvent("DenaliFW:Notify",_source,"You added $~g~"..amount.."~s~ to the shops funds.")

  else

    TriggerClientEvent("DenaliFW:Notify",_source,"You can't afford that.")
...
TakeFunds = function(shop_key,amount)
  ...
  if VehicleShops[shop_key].funds >= amount then
    ...
    if identifier == VehicleShops[shop_key].owner then
      ...
      TriggerClientEvent("DenaliFW:Notify",_source,"You took $~r~"..amount.."~s~ from the shops funds.")

    end

  else

    TriggerClientEvent("DenaliFW:Notify",_source,"The shop doesn't have that many funds.")
...
HirePlayer = function(shop_key,target_id)
  ...
  local mPlayer = DenaliFW.Functions.GetPlayer(source)
  ...
  if shop and shop.owner and shop.owner == mIdentifier then
    ...
    local xPlayer = DenaliFW.Functions.GetPlayer(target_id)
    ...
    for k,v in pairs(VehicleShops[shop_key].employees) do
      ...
      if v.identifier == target_id then
        ...
        if KashPlayers[target_id] then
          ...
          xPlayer = DenaliFW.Functions.GetPlayer(KashPlayers[target_id].src)

        else

          xPlayer = DenaliFW.Functions.GetPlayerentifier(target_id)
        ...
        if xPlayer then
          ...
          TriggerClientEvent("DenaliFW:Notify",source,string.format("Payed %s %s $%i.",v.identity.firstname,v.identity.lastname,amount))
          ...
          TriggerClientEvent("DenaliFW:Notify",source,"Player is not online.")
...
DenaliFW.Functions.CreateCallback("VehicleShops:GetVehicleShops", GetVehicleShops)

DenaliFW.Functions.CreateCallback("VehicleShops:GetVehicleOwner", GetVehicleOwner)

DenaliFW.Functions.CreateCallback("VehicleShops:GenerateNewPlate", GenerateNewPlate)

DenaliFW.Functions.CreateCallback("VehicleShops:TryBuy", TryBuy)

DenaliFW.Functions.CreateCallback("VehicleShops:PurchaseShop", PurchaseShop)

DenaliFW.Functions.CreateCallback("VehicleShops:DriveVehicle", DriveVehicle)
