...
function isAdmin(source)
    local source = source
    return DenaliFW.Functions.IsOptin(source)
end

DenaliFW.Functions.CreateCallback('renzu_scoreboard:playerlist', function(source, cb)
    local list = {}
    local whitelistedjobs = {}
    local source = tonumber(source)
    for k,v in pairs(players) do
        local Player = DenaliFW.Functions.GetPlayer(tonumber(v.id))
    ...
    Player = DenaliFW.Functions.GetPlayer(source)
    if Player ~= nil then
