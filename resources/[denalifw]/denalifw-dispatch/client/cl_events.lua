...
RegisterNetEvent("denalifw-dispatch:client:officerdown", function()
    local plyData = DenaliFW.Functions.GetPlayerData()
    local currentPos = GetEntityCoords(PlayerPedId())
    local locationInfo = getStreetandZone(currentPos)
    local callsign = DenaliFW.Functions.GetPlayerData().metadata["callsign"]
    TriggerServerEvent("dispatch:server:notify",{
...
RegisterNetEvent("denalifw-dispatch:client:emsdown", function()
    local plyData = DenaliFW.Functions.GetPlayerData()
    local currentPos = GetEntityCoords(PlayerPedId())
    local locationInfo = getStreetandZone(currentPos)
    local callsign = DenaliFW.Functions.GetPlayerData().metadata["callsign"]
