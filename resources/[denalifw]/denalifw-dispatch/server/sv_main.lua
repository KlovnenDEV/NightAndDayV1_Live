...
AddEventHandler("dispatch:server:notify", function(data)
    ...
    TriggerClientEvent("denalifw-dispatch:client:AddCallBlip", -1, data.origin, dispatchCodes[data.dispatchcodename])
end)
...
AddEventHandler("dispatch:sendCallResponse", function(player, callid, message, time, cb)
    local Player = DenaliFW.Functions.GetPlayer(player)
