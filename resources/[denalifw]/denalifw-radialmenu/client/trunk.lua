...
RegisterNetEvent('denalifw-trunk:client:KidnapTrunk', function()
    ...
    if (distance ~= -1 and distance < 2) then
        if isKidnapping then
            ...
            DenaliFW.Functions.Notify('You did not kipnap this person!', 'error')
        end
    end
end)
                    ...
                    if not isBusy then
                        if not isKidnapped then
                            -- if GetVehicleDoorAngleRatio(closestVehicle, 5) > 0 then
                                ...
                                DenaliFW.Functions.Notify('You\'re in the trunk.', 'success', 4000)
                                TrunkCam(true)

                                isKidnapped = true
                            -- else
                                -- DenaliFW.Functions.Notify('Is the trunk closed?', 'error', 2500)
                            -- end
                        else
                            local ped = PlayerPedId()
                            local vehicle = GetEntityAttachedTo(PlayerPedId())
                            ...
                            if GetVehicleDoorAngleRatio(vehicle, 5) > 0 then
                                ...
                                DenaliFW.Functions.Notify('The trunk is closed?', 'error', 2500)
                            end
                        end
                    else
                        DenaliFW.Functions.Notify('Anyone in there yet?', 'error', 2500)
                    end
                else
                    DenaliFW.Functions.Notify('Your already in the trunk', 'error', 2500)
                end
            else
                DenaliFW.Functions.Notify('You cant get in this trunk..', 'error', 2500)
            end
        end, plate)
    else
        DenaliFW.Functions.Notify('You cant get in this trunk..', 'error', 2500)
...
RegisterNetEvent('denalifw-trunk:client:GetIn', function(isKidnapped)
    local ped = PlayerPedId()
    local closestVehicle = getNearestVeh()
    ...
    if closestVehicle ~= 0 then
                    ...
                    if not inTrunk then
                        if not isBusy then
                            if GetVehicleDoorAngleRatio(closestVehicle, 5) > 0 then
                                ...
                                Wait(500)
                                SetVehicleDoorShut(closestVehicle, 5, false)
                                DenaliFW.Functions.Notify('You are already in the trunk.', 'Goodluck', 4000)
                                TrunkCam(true)
                            else
                                DenaliFW.Functions.Notify('Is the trunk closed?', 'error', 2500)
                            end
                        else
                            DenaliFW.Functions.Notify('Looks like there is some 1 in there?', 'error', 2500)
                        end
                    else
                        DenaliFW.Functions.Notify('You are already in the trunk', 'error', 2500)
                    end
                else
                    DenaliFW.Functions.Notify('You cant get in this trunk..', 'error', 2500)
                end
        ...
        DenaliFW.Functions.Notify('There is no vehicle to see.', 'error', 2500)
    end
end)
...
AddEventHandler('qb-phone:client:GiveContactDetails', function()
    ...
    if player ~= -1 and distance < 2.5 then
        ...
        DenaliFW.Functions.Notify("No one around!", "error")
    end
end)
