...
RegisterNetEvent('denalifw-radialmenu:client:TakeStretcher', function()
    ...
    if Vehicle ~= 0 then
        ...
        if Obj ~= nil or Obj ~= 0 then
            ...
            DenaliFW.Functions.Notify("Something went wrong..", 'error')
        end
    else
        DenaliFW.Functions.Notify("You're not near an Ambulance..", 'error')
    end
end)
    ...
    if StretcherObject ~= nil then
        ...
        if Dist < 3.0 then
            if DoesEntityExist(StretcherObject) then
                ...
                TriggerServerEvent('denalifw-radialmenu:server:RemoveStretcher', PlayerPos, StretcherObject)
                IsAttached = false
                StretcherObject = nil
                IsLayingOnBed = false
            end
        else
            DenaliFW.Functions.Notify('You\'re too far away!', 'error')
        end
    end
end)
...
RegisterNetEvent('denalifw-radialmenu:client:Result', function(IsBusy, type)
    ...
    if type == "lay" then
        if not IsBusy then
            ...
            DenaliFW.Functions.Notify("This stretcher is already in use!", "error")
            IsLayingOnBed = false
        end
    else
        if not IsBusy then
