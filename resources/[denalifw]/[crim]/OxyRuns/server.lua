...
AddEventHandler("oxyrun:server:deliver", function()
	...
	if hasPackage then
		...
		if Config.GiveItem then
			...
			TriggerClientEvent('inventory:client:ItemBox', source, DenaliFW.Shared.Items[Config.ItemReward], "add")
		end
		...
		if Config.CleanMoney then
			...
			if chance < Config.BagChance then
				if hasBags then
					...
					TriggerClientEvent('inventory:client:ItemBox', source, DenaliFW.Shared.Items["markedbills"], "remove")
				end
			end
			...
			if chance < Config.BandChance then
				if hasBands then
					...
					TriggerClientEvent('inventory:client:ItemBox', source, DenaliFW.Shared.Items["bands"], "remove")
				end
			end
			...
			if chance < Config.RollChance then
				if hasRolls then
					...
					TriggerClientEvent('inventory:client:ItemBox', source, DenaliFW.Shared.Items["rolls"], "remove")
				end
			end
		end
	
		Player.Functions.RemoveItem("package", 1)
		TriggerClientEvent('inventory:client:ItemBox', source, DenaliFW.Shared.Items['package'], "remove")
	end
end)
...
DenaliFW.Functions.CreateCallback('oxyrun:server:hasPackage', function(source, cb)
    local src = source
...
DenaliFW.Functions.CreateCallback('oxyrun:server:canCarry', function(source, cb)
    local src = source
    local Player = DenaliFW.Functions.GetPlayer(src)
 
	if Player.Functions.AddItem("package", 1) then
		cb(true)
		TriggerClientEvent('inventory:client:ItemBox', source, DenaliFW.Shared.Items['package'], "add")
	else
		cb(false)
	end
end)