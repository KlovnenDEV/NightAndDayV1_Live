...
AddEventHandler("oxyrun:startCar", function(pedNetId, route)
	...
	exports['denalifw-target']:AddTargetEntity(car, {
		options = {
			{
				type = "client",
...
AddEventHandler("oxyrun:client:deliverPackage", function()
	if holdingBox then
		if currentCars <= deliveries then
			...
			TriggerEvent('DenaliFW:Notify', 'Well done, (' .. currentCars .. '/' .. deliveries .. ')')
		end
		...
		DenaliFW.Functions.Notify(Config.Text["nopackage"], 'error', 5000)
	end
end)
...
AddEventHandler("oxyrun:client:hasPackage", function()
	...
	DenaliFW.Functions.TriggerCallback('oxyrun:server:hasPackage', function(hasItems)
		if hasItems then
	...
	while true do
		...
		if gettingVehicle then
			if IsPedInAnyVehicle(PlayerPedId(), false) then
				gettingVehicle = false
				TriggerEvent('DenaliFW:Notify', Config.Text["getboxes"])
...
RegisterNetEvent('oxyrun:client:sendToOxy', function()
	...
	-- 	DenaliFW.Functions.Notify(Config.Text["noroutes"], 'error', 5000)
	-- 	return
	-- end
	...
	TriggerEvent('DenaliFW:Notify', Config.Text["stealveh"])
	gettingBox = true
	gettingVehicle = true
end)
...
RegisterNetEvent('oxyrun:client:getBox', function()
	if gettingBox then
		...
		DenaliFW.Functions.TriggerCallback('oxyrun:server:canCarry', function(canCarry)
			if canCarry then
				if currentBoxes < amountOfBox then
					TriggerEvent("oxyrun:client:hasPackage")
					currentBoxes = currentBoxes + 1
					TriggerEvent('DenaliFW:Notify', '('.. currentBoxes .. '/' .. amountOfBox .. ')')
					if currentBoxes == amountOfBox then
						TriggerEvent('DenaliFW:Notify', Config.Text["drivetohandoff"])
		...
		DenaliFW.Functions.Notify("Something seems wrong...", 'error', 5000)
	end
end)
    ...
    if LocalPlayer.state["isLoggedIn"] then
        if name == "AtOxy" then
			...
			if doingOxy and atOxy and not zoneEntered then
				TriggerEvent("oxyrun:client:sendCar")
				TriggerEvent('DenaliFW:Notify', Config.Text["atlocation"])
				zoneEntered = true
			end
        end  
    end
...
RegisterNetEvent('oxyrun:client:endOxy', function()  
	...
	TriggerEvent('DenaliFW:Notify', Config.Text["done"])
	RemoveBlip(oxyBlip)
end)
