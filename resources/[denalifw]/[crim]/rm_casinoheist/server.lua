...
DenaliFW.Functions.CreateCallback('casinoheist:server:checkTime', function(source, cb)
    local src = source
    ...
    if (os.time() - CasinoHeist['lastHeist']) < Config['CasinoHeist']['nextHeist'] and CasinoHeist['lastHeist'] ~= 0 then
        ...
        TriggerClientEvent('DenaliFW:Notify', src, Strings['wait_nextheist'] .. ' ' .. math.floor(seconds / 60) .. ' ' .. Strings['minute'], "error")
		cb(false)
    else
...
DenaliFW.Functions.CreateCallback('casinoheist:server:hasItem', function(source, cb, item)
    local src = source
    local player = DenaliFW.Functions.GetPlayer(src)
    ...
    if player then
        if CasinoHeist['start'] then
            if reward.item ~= nil then
				...
				TriggerClientEvent('inventory:client:ItemBox', src, DenaliFW.Shared.Items[reward.item], "add")
            else
