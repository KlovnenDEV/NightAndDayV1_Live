...
local Beds, CurrentBed, OnBed = {"prop_rub_couch04"}, nil, false
      ...
      for k,v in ipairs(Config.Doctor) do
          if (GetDistanceBetweenCoords(coords, v.x, v.y, v.z, true) < 2.0) then     
              ...
              elseif IsControlJustReleased(0, 38) and IsEntityDead(playerPed) and CurrentBed ~= nil then
                ...
                DenaliFW.Functions.Notify("You are not injured", "error")
              end
            -- end)
          end
      end
...
DenaliFW.Functions.Progressbar("use_bandage", "You are being treated", 60000, false, true, {
...
function getOnBed() 
  local PlayerPed = PlayerPedId()
      ...
      local coords = vector3(786.63, 4178.8, 41.19)
      print(BedCoords)
...
function createCameraScene()
  local cam = CreateCam("DEFAULT_SCRIPTED_CAMERA", 1)
	SetCamCoord(cam, 783.35, 4178.24, 41.89)
