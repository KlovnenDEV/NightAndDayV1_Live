...
DenaliFW.Functions.CreateCallback('trainheist:server:checkPoliceCount', function(source, cb)
    local src = source
    local players = DenaliFW.Functions.GetPlayers(src)
    local policeCount = 0
    ...
    if policeCount >= Config['TrainHeist']['requiredPoliceCount'] then
        ...
        cb(false)
        TriggerClientEvent('DenaliFW:Notify', src, Strings['need_police'], "error")
    end
end)

DenaliFW.Functions.CreateCallback('trainheist:server:checkTime', function(source, cb)
    local src = source
    ...
    if (os.time() - lastrob) < Config['TrainHeist']['nextRob'] and lastrob ~= 0 then
        ...
        TriggerClientEvent('DenaliFW:Notify', src, Strings['wait_nextrob'] .. ' ' .. math.floor(seconds / 60) .. ' ' .. Strings['minute'], "error")
        cb(false)
    else
        lastrob = os.time()
        start = true
...
DenaliFW.Functions.CreateCallback('trainheist:server:hasItem', function(source, cb, item)
    local src = source
    local player = DenaliFW.Functions.GetPlayer(src)
    ...
    if player and playerItem ~= nil then
        ...
        TriggerClientEvent('DenaliFW:Notify', src, 'You need add required items to server database', "error")
    end
end)

RegisterServerEvent('trainheist:server:policeAlert')
AddEventHandler('trainheist:server:policeAlert', function(coords)
    local players = DenaliFW.Functions.GetPlayers(source)
...
AddEventHandler('trainheist:server:rewardItems', function()
    ...
    if player then
		...
		TriggerClientEvent('inventory:client:ItemBox', src, DenaliFW.Shared.Items[Config['TrainHeist']['reward']['itemName']], "add")
    end
end)
...
AddEventHandler('trainheist:server:sellRewardItems', function()
    ...
    if player then
        ...
        if cnt > 0 then
            ...
            TriggerClientEvent('DenaliFW:Notify', src, Strings['total_money'] .. ' $' .. Config['TrainHeist']['reward']['sellPrice'] * cnt, "error")
        end
    end
end)
