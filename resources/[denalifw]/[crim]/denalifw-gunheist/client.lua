...
local hackingPosition = vector3(-1423.4, 6757.74, 5.8)
...
RegisterNetEvent('lrp-gunheist:manipulateDrop')
AddEventHandler('lrp-gunheist:manipulateDrop', function(dropId, dropData)
...
RegisterNetEvent('lrp-gunheist:obtainDrops')
AddEventHandler('lrp-gunheist:obtainDrops', function(serverDrops)
	activeDrops = serverDrops
end)

RegisterNetEvent('lrp-gunheist:createBlipOnDrop')
AddEventHandler('lrp-gunheist:createBlipOnDrop', function(dropId)
...
RegisterNetEvent('lrp-gunheist:openCrate')
AddEventHandler('lrp-gunheist:openCrate', function(crateObject)
...
Citizen.CreateThread(function()
	TriggerServerEvent('lrp-gunheist:obtainDrops')
	...
	while true do
		...
		if dropDistance < 2.5 then
			...
			if dropDistance < 1.5 then
				...
				TriggerEvent('cd_drawtextui:ShowUI', 'show', 'Drill Crate')

				if IsControlJustReleased(0, 38) then
					TriggerServerEvent('checkDrill', dropId)
					TriggerServerEvent('ghesit', exports['cd_dispatch']:GetPlayerInfo())
				end
			else
...
AddEventHandler('do', function(dropId)
	...
	TriggerEvent('lrp-gunheist:openCrate', activeDrops[dropId].object)
	TriggerServerEvent('gheist', exports['cd_dispatch']:GetPlayerInfo())
	
	-- local finished = exports["op-progressbar"]:progressBar("Drilling Crate", math.random(20000, 30000))
	DenaliFW.Functions.Progressbar("drilling_", "Drilling...", math.random(60000, 70000), false, true, {}, {}, {}, {}, function()
		TriggerServerEvent('lrp-gunheist:pickUpDrop', dropId)
		inAnimation = false
	end)
end) 
...
RegisterNetEvent('lrp-gunheist:useChip')
AddEventHandler('lrp-gunheist:useChip', function()
	local dstcheck = #(GetEntityCoords(PlayerPedId()) - hackingPosition)
	
	if dstcheck < 5 then
		DrawText3Ds(coords.x, coords.y, coords.z, "Drill Crate")
		if StartHacking() then
			TriggerServerEvent('lrp-gunheist:wonHacking')
		end
	end
end)
...
AddEventHandler('lrp-gunheist:doDecryptionAnimation', function()
	local ped = PlayerPedId()
	RequestAnimDict('missheist_jewel@hacking')
	TaskPlayAnim(ped, "missheist_jewel@hacking", "hack_loop", 8.0, 1.0, -1, 9, 0, 0, 0, 0)
end)

RegisterNetEvent('lrp-gunheist:useDecryptionKey')
AddEventHandler('lrp-gunheist:useDecryptionKey', function(item)
	local dstcheck = #(GetEntityCoords(PlayerPedId()) - hackingPosition)
	TriggerEvent('cd_drawtextui:ShowUI', 'show', 'Use Red Blue Or Green Chip')
	if dstcheck < 5 then
		TriggerEvent('lrp-gunheist:doDecryptionAnimation')


		DenaliFW.Functions.Progressbar("drilling_", "Decrypting with key...", math.random(10000, 15000), false, true, {}, {}, {}, {}, function()
