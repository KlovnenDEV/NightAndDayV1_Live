...
RegisterNetEvent('DenaliFW:Client:OnPlayerLoaded', function()
    ...
    onDuty = true
    TriggerServerEvent('nv-carboost:server:getItem')
    TriggerEvent('nv-carboost:client:setupBoostingApp')
end)
...
RegisterNetEvent('DenaliFW:Client:OnPlayerUnload', function ()
    PlayerJob = {}
    TriggerServerEvent('nv-carboost:server:removeData', PlayerData.citizenid)  
    PlayerData = {}
    Config.BennysItems = {}
end)
        ...
        if not veh.state.hacked then
            ...
            if success then
                ...
                if trackerLeft == 0 then
                    ...
                    veh.state.hacked = true
                    return DenaliFW.Functions.Notify(Lang:t("success.disable_tracker"))
                else
                    DenaliFW.Functions.Notify(Lang:t('success.tracker_off', {time = randomSeconds, tracker_left = trackerLeft}))
                end
                CreateThread(function ()
                ...
                DenaliFW.Functions.Notify(Lang:t('error.disable_fail'), "error")
            end
        else
            print("ALREADY HACKED")
        end
    else
        DenaliFW.Functions.Notify(Lang:t("error.no_tracker"), "error")
    end
end
...
local function finishBoosting(data)
    TriggerServerEvent('nv-carboost:server:finishBoosting', 'normal', Tier)
    TriggerEvent('nv-carboost:client:deleteContract')
    ...
    ContractID = nil
    Wait(1000)
    TriggerEvent('nv-carboost:client:refreshQueue')
end

local function Scratching()
    TriggerServerEvent('nv-carboost:server:finishBoosting', 'vin', Tier)
    TriggerEvent('nv-carboost:client:deleteContract')
...
local function BoostingAlert()
    if Config.Alert == 'denalifw-dispatch' then
        AlertBoosting(carID, 'denalifw-dispatch')
    ...
    if Config.BennysSell then
        for k, v in pairs(Config.BennysSell) do
            ...
            if not storeitem[k] then
                if DenaliFW.Shared.Items[v.item] ~= nil then
                    if Config.BennysSell[k].name then
                        name = Config.BennysSell[k].name
                    else
                       if DenaliFW.Shared.Items[v.item] ~= nil then
                           name = DenaliFW.Shared.Items[v.item].label
                       else
                        name = "Unknown"
                         return TriggerServerEvent('nv-carboost:server:log', "")
                       end
                    end
                    storeitem[#storeitem+1] = {
                        name = name,
                        item = v.item,
                        image = Config.Inventory ..DenaliFW.Shared.Items[v.item].image,
                        price = v.price,
                        stock = v.stock
                    }
                else
                    TriggerServerEvent('nv-carboost:server:log', "The item is not found :"..k)
                end
            else
                TriggerServerEvent("nv-carboost:server:log", "Duplicate item found: " .. k)
            end
        end
        cb({
            storeitem = storeitem
        ...
        TriggerServerEvent('nv-carboost:server:log', "The store is empty")
        cb({
...
RegisterNUICallback('canStartContract', function (data, cb)
    ...
    if isContractStarted then
        ...
        if data.type == 'vin' then
            DenaliFW.Functions.TriggerCallback('nv-carboost:server:vinmoney', function(result)
...
RegisterNUICallback('startcontract', function (data)
    ...
    if not isContractStarted then
        ...
        Tier = data.data.tier
        DenaliFW.Functions.TriggerCallback('nv-carboost:server:getContractData', function (result)
            if result then
                TriggerEvent('nv-carboost:client:spawnCar', result)
            else
                print("NO RESULT")
            end
        end, data)
...
RegisterNUICallback('stopcontract', function (data)
    if isContractStarted then
        isContractStarted = false
        TriggerEvent('nv-carboost:client:stopContract')
    end
end)

RegisterNUICallback('checkout', function (data)
    DenaliFW.Functions.TriggerCallback('nv-carboost:server:canBuy', function (result)
        if result then
            ...
            TriggerServerEvent('nv-carboost:server:buyItem', itemData.price, Config.BennysItems, firstTime)
            SendNUIMessage({
                type="checkout",
                success = result
            })
...
RegisterNUICallback('setupboostapp', function (data, cb)
    DenaliFW.Functions.TriggerCallback('nv-carboost:server:getboostdata', function (result)
        if result then
...
RegisterNUICallback('sellcontract', function(data, cb)
    DenaliFW.Functions.TriggerCallback('nv-carboost:server:sellContract', function (result)
        if result then
            cb(result)
        else
            cb({
...
RegisterNUICallback('transfercontract', function (data, cb)
    DenaliFW.Functions.TriggerCallback('nv-carboost:server:transfercontract', function (result)
        if result and not result.error then
            cb({
                success = result
            })
...
RegisterNUICallback('joinqueue', function (data)
    TriggerEvent('nv-carboost:client:joinQueue', data)
end)

RegisterNUICallback('buycontract', function(data, cb)
    DenaliFW.Functions.TriggerCallback('nv-carboost:server:buycontract', function(result)
        if result then
            cb(result)
        end
    end, data)
...
RegisterNetEvent('nv-carboost:client:setConfig', function (data)
    Config.BennysItems = data
end)

RegisterNetEvent('nv-carboost:client:joinQueue', function (data)
    isJoinQueue = data.status
    local citizenid = PlayerData.citizenid
    TriggerServerEvent('nv-carboost:server:joinQueue', isJoinQueue, citizenid)
end)

RegisterNetEvent('nv-carboost:client:refreshQueue', function ()
    local citizenid = PlayerData.citizenid
    if isJoinQueue then
        TriggerServerEvent('nv-carboost:server:joinQueue', false, citizenid)
        Wait(2000)
        TriggerServerEvent('nv-carboost:server:joinQueue', true, citizenid)
    end
end)

RegisterNetEvent('nv-carboost:client:giveContract', function ()
    SendNUIMessage({
        type="givecontract",
        data = {
...
RegisterNetEvent('nv-carboost:client:setupBoostingApp', function ()
    SendNUIMessage({
        type="setupboostingapp",
    })
end)

RegisterNetEvent('nv-carboost:client:newContract', function ()
    TriggerServerEvent('nv-carboost:server:newContract', PlayerData.citizenid)
end)

RegisterNetEvent('nv-carboost:client:addContract', function (data)
...
RegisterNetEvent('nv-carboost:client:takeAll', function ()
    TriggerServerEvent('nv-carboost:server:takeAll', Config.BennysItems)
    Config.BennysItems = {}
end)

RegisterNetEvent('nv-carboost:client:takeItem', function (data)
    TriggerServerEvent('nv-carboost:server:takeItem', data.item, data.quantity)
    ...
    TriggerServerEvent('nv-carboost:server:updateBennysConfig', Config.BennysItems)
end)

RegisterNetEvent('nv-carboost:client:openMenu', function ()
    if Config.BennysItems[1] then
        ...
        for _, v in pairs(Config.BennysItems) do
            ...
            menu[#menu+1] = {
                header = DenaliFW.Shared.Items[item.name]["label"],
                id = item.name,
                txt = "You have: "..item.quantity,
                params = {
                    event = "nv-carboost:client:takeItem",
                    args = {
                            item = item.name,
        ...
        menu[#menu+1] = {
            ...
            params = {
                event = "nv-carboost:client:takeAll"
            }

        }
        exports['denalifw-menu']:openMenu(menu)
    else
        DenaliFW.Functions.Notify(Lang:t("error.empty_post"), "error")
    end
end)

RegisterNetEvent('nv-carboost:client:spawnCar', function(data)
    DenaliFW.Functions.TriggerCallback('nv-carboost:server:spawnCar', function(result)
        if result then
            ...
            TriggerServerEvent('denalifw-phone:server:sendNewMail', {
             ...
             TriggerEvent('nv-carboost:client:startBoosting', result)
        end
    end, data)
end)

RegisterNetEvent('nv-carboost:client:startBoosting', function (data)
    local data = data
    ContractID = data.id
            ...
            if carID ~= nil then
                if DoesEntityExist(NetworkGetEntityFromNetworkId(carID)) then
                    ...
                    if not modified then
                        DenaliFW.Functions.SetVehicleProperties(carSpawned, VehProp)
                        modified = true
                    end
                    ...
                    if dist <= 5.0 then
                        if blipDisplay ~= nil then
                            ...
                            TriggerEvent('nv-carboost:client:playerInVehicle', data)
                            break
                        end
                    end
                end
...
RegisterNetEvent('nv-carboost:client:checkvin', function ()
    ...
    if vehicle ~= 0 then
        ...
        if #(PlayerPos - vehPos) <= 5.0 then
            ...
            if GetVehicleDoorLockStatus(vehicle) == 1 then
                    DenaliFW.Functions.Progressbar('check_vin', 'Checking VIN Number', 6000, false, true, { -- Name | Label | Time | useWhileDead | canCancel
                        ...
                        DenaliFW.Functions.TriggerCallback('nv-carboost:server:checkvin', function(result)
                            if result and result.success then
    
                                DenaliFW.Functions.Notify(result.message, "primary")
                               
                            else
                                DenaliFW.Functions.Notify("Hmm you can't found the VIN", "error")
                            end
                        end, networkID)
                    ...
                    DenaliFW.Functions.Notify("The vehicle is locked", "error")
                end
            else
...
RegisterNetEvent('nv-carboost:client:vinscratch', function(veh)
    local ID = NetworkGetNetworkIdFromEntity(veh)
    DenaliFW.Functions.Progressbar('vin_scratching', 'Scratching VIN', 7000, false, true, { -- Name | Label | Time | useWhileDead | canCancel
        disableMovement = true,
        disableCarMovement = true,
        disableMouse = false,
        disableCombat = true,
        ...
        DenaliFW.Functions.TriggerCallback('nv-carboost:server:checkvin', function(result)
            if result and result.owner == PlayerData.citizenid then
                if result.vinscratch == 1 then
                    return DenaliFW.Functions.Notify('You already scratch this vehicle VIN', 'error')
                end
            else
                DenaliFW.Functions.Notify('VIN Scratched, you can change your plate number', 'primary')
                local vehProp = DenaliFW.Functions.GetVehicleProperties(veh)
                TriggerServerEvent('nv-carboost:server:vinscratch', ID, vehProp, carmodel)
                TriggerEvent('nv-carboost:client:deleteContract')
                Scratching()
            end
        end, ID)
...
RegisterNetEvent('nv-carboost:client:playerInVehicle', function (data)
    local data = data
    CreateThread(function ()
        while true do
            Wait(100)
            if IsPedInVehicle(PlayerPedId(), carSpawned, false) then
                if IsVehicleAlarmActivated(carSpawned) or IsVehicleEngineStarting(carSpawned) or GetIsVehicleEngineRunning(carSpawned) then       
                    ...
                    TriggerEvent('nv-carboost:client:trackersReady', data)
                    break
                end
            end
        end
...
RegisterNetEvent('nv-carboost:client:trackersReady', function (data)
    CreateThread(function ()
        while true do
            Wait(1000)
            if GetIsVehicleEngineRunning(carSpawned) then
                ...
                BoostingAlert()
                TriggerEvent('nv-carboost:client:startTracker', data)
                break
            end
        end
    end)
...
RegisterNetEvent('nv-carboost:client:startTracker', function(data)
    local veh = Entity(carSpawned)
    CreateThread(function ()
        while true do
            Wait(5000)
            if not veh.state.tracker then
                TriggerEvent("nv-carboost:client:bringtoPlace", data)
                break
            end
        end
    end)
...
RegisterNetEvent("nv-carboost:client:bringtoPlace", function (data)
    ...
    if data.type == 'vin' then
        ...
        scratchpoint:onPointInOut(function ()
            ...
            if inscratchPoint then
                DenaliFW.Functions.Notify(Lang:t("info.in_scratch"))
            else
                DenaliFW.Functions.Notify(Lang:t("info.not_in_scratch"))
            end
        end)
    else
        ...
        zone:onPointInOut(function ()
            ...
            if inZone then
                DenaliFW.Functions.Notify(Lang:t("info.car_inzone"))
            end
        end)
        CreateThread(function ()
            while true do
...
RegisterNetEvent('nv-carboost:client:finishBoosting', function (data)

end)

RegisterNetEvent('nv-carboost:client:updateProggress', function (data)
    local data = {
...
RegisterNetEvent('nv-carboost:client:deleteContract', function ()
    SendNUIMessage({
        type = "removeContract",
        id = ContractID
    })
    TriggerServerEvent('nv-carboost:server:deleteContract', ContractID)
end)

RegisterNetEvent('nv-carboost:client:playerUnload', function()
    if carSpawned then
        DeleteVehicle(carSpawned)
    end
    TriggerServerEvent('nv-carboost:server:joinQueue', false, PlayerData.citizenid)
end)

RegisterNetEvent('nv-carboost:client:failedBoosting', function ()
    if zone then
        zone:destroy()
        zone = nil
        inZone = false
    ...
    isContractStarted = false
    DenaliFW.Functions.Notify(Lang:t("error.no_car"), "error")
    TriggerEvent('nv-carboost:client:refreshQueue')
    TriggerEvent('nv-carboost:client:deleteContract')
end)

RegisterNetEvent('nv-carboost:client:openLaptop', function ()
    SetDisplay(not display)
end)

RegisterNetEvent('nv-carboost:client:loadBoostStore', function(data)
    local cdata = data
    local saleData = {}
...
RegisterNetEvent('nv-carboost:client:refreshContract', function ()
    DenaliFW.Functions.TriggerCallback('nv-carboost:server:getContractData')
    SendNUIMessage({
        type = "refreshContract",
    })
end)

RegisterNetEvent('nv-carboost:client:newContractSale', function(data)
    local data = data 
...
RegisterNetEvent('nv-carboost:client:contractBought', function(id)
    SendNUIMessage({
        type = "contractbought",
        id = id
    })
...
RegisterNetEvent('nv-carboost:client:useHackingDevice', function ()
    ...
    if IsPedInVehicle(playerPed, vehicle) then
        if GetPedInVehicleSeat(GetVehiclePedIsIn(PlayerPedId(), false), 0) ~= 0 then
            if cooldown then
                return DenaliFW.Functions.Notify(Lang:t("error.cannot_use"), "error")
            end
            ...
            return DenaliFW.Functions.Notify(Lang:t('error.not_seat'), "error")
        end
    else
        DenaliFW.Functions.Notify(Lang:t("error.not_on_vehicle"), "error")
    end
end)

RegisterNetEvent('nv-carboost:client:fakeplate', function()
    ...
    if veh ~= 0 and not IsPedInAnyVehicle(PlayerPedId()) then
        if distFront < 2.0 or distBack < 2.0 then 
            DenaliFW.Functions.TriggerCallback('nv-carboost:server:checkvin', function (result)
                if result then
                    if result.owner == PlayerData.citizenid then
                        DenaliFW.Functions.Progressbar('change_plate', 'Changing the plate', 8000, false, true, { -- Name | Label | Time | useWhileDead | canCancel
                            disableMovement = true,
                            disableCarMovement = true,
                            disableMouse = false,
                            disableCombat = true,
                            ...
                            TriggerEvent('nv-carboost:client:setPlate', vehID)
                            ClearPedTasks(PlayerPedId())
                        end, function() -- Play When Cancel
                            ClearPedTasks(PlayerPedId())
                            --Stuff goes here
            ...
            DenaliFW.Functions.Notify(Lang:t("error.not_plate"), "error")
        end
    end
end)
...
RegisterNetEvent('nv-carboost:client:setPlate', function (vehID)
    ...
    if veh and veh ~= 0 then
        ...
        TriggerServerEvent('nv-carboost:server:setPlate', plate, newPlate)
        DenaliFW.Functions.Notify(Lang:t("success.plate_changed", {
            plate = newPlate
        }), "success")
        TriggerEvent('inventory:client:ItemBox', DenaliFW.Shared.Items["fake_plate"], "remove")
        TriggerServerEvent("DenaliFW:Server:RemoveItem", "fake_plate", 1)
...
CreateThread(function ()
    if LocalPlayer.state['isLoggedIn'] then
        Wait(5000)
        TriggerServerEvent('nv-carboost:server:getItem')
        TriggerServerEvent('nv-carboost:server:getBoostSale')
        TriggerEvent('nv-carboost:client:setupBoostingApp')
    end
    ...
    while true do
        ...
        if carSpawned ~= nil and not DoesEntityExist(carSpawned) then
            if carSpawned ~= 0 then
                TriggerEvent('nv-carboost:client:failedBoosting')
            end
        end
    end
end)
...
-- exports
exports['denalifw-target']:AddBoxZone("carboost:takeItem", vector3(1185.14, -3304.01, 7.1), 2, 2, {
	...
	options = {
		{
            type = "client",
            event = "nv-carboost:client:openMenu",
...
exports['denalifw-target']:AddTargetBone('windscreen', {
    options = {
        {
            icon = "fas fa-tally",
            label = "Scratch VIN",
            ...
            action = function (entity)
                TriggerEvent('nv-carboost:client:vinscratch', entity)
            end
        },
    },
    distance = 1.2
    ...
    options = {
        {
            ...
            label = "Check VIN",
            event = "nv-carboost:client:checkvin",
            job = "police",
        },
    },
    distance = 2
