...
Citizen.CreateThread(function()
  ...
  local illlegalHunts = {
	...
	exports['denalifw-target']:AddTargetModel(legalHunts, {
    options = {
      {
...
RegisterNetEvent('hunting:shop', function (ShopItems)
	...
	DenaliFW.Functions.TriggerCallback("denalifw-shops:server:getLicenseStatus", function(hasLicense, hasLicenseItem)
		ShopItems.label = "hunting shop | GUN LICENSE VERIFIED"
		if hasLicense and hasLicenseItem then
			...
			local ShopItems = {}
			ShopItems.label = "hunting shop | NO LICENSE"
			ShopItems.items = Config.NoLicense
			ShopItems.slots = #Config.NoLicense
			TriggerServerEvent("inventory:server:OpenInventory", "shop", "NoLicense_"..math.random(1, 99), ShopItems) 
		end
	end)
end)
...
AddEventHandler('Dox-hunting:spawnAnimal', function()
  ...
  if outPosition.x > 1 or outPosition.x < -1  then
    ...
    DenaliFW.Functions.Notify('you lost a one bait because it to far from the zone learn for your mistake :(')
  end
...
AddEventHandler('Dox-hunting:skinAnimal', function()
  DenaliFW.Functions.TriggerCallback('DenaliFW:HasItem', function(hasItem)
    if hasItem then
      ...
      DenaliFW.Functions.Progressbar('name_here', 'skinning ...', 5000, false, true, { -- Name | Label | Time | useWhileDead | canCancel
      ...
      DeleteEntity(baitAnimal)
        ...
        DenaliFW.Functions.Notify('not your shit !!!', 'error')
        print('Not your shit Bitch')
      end
    else
      DenaliFW.Functions.Notify("You need a knife", "error")
    end
  end, "WEAPON_KNIFE")
end)
...
AddEventHandler('Dox-hunting:usedBait', function()
  for k,v in pairs (HuntingZones) do
    if IsEntityInZone(PlayerPedId(),v) then
      if cooldown <= 0 then
        if IsPedInAnyVehicle(PlayerPedId(), false) then
          DenaliFW.Functions.Notify('you cant use that in any car', 'error')
        else
        LoadAnimDict('amb@medic@standing@kneel@base')
        TaskPlayAnim(PlayerPedId(), "amb@medic@standing@kneel@base" ,"base" ,8.0, -8.0, -1, 1, 0, false, false, false )
        DenaliFW.Functions.Progressbar('name_here', 'Placing Bait ...', 5000, false, true, { -- Name | Label | Time | useWhileDead | canCancel
        disableMovement = true,
        disableCarMovement = true,
        disableMouse = false,
        disableCombat = true,
    ...
    --  TriggerEvent("DenaliFW:Notify", "UHMMM FIND SOMETHING TO WRITE HERE",2)
    end
  end
end
end)
