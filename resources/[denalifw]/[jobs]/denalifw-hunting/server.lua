...
AddEventHandler('Dox-hunting:skinReward', function()
  ...
  if randomAmount > 1 and randomAmount < 15 then
    ...
    TriggerClientEvent('inventory:client:ItemBox', src, DenaliFW.Shared.Items["huntingcarcass1"], "add")

  elseif randomAmount > 15 and randomAmount < 23 then

    Player.Functions.AddItem("huntingcarcass2", 1)
    TriggerClientEvent('inventory:client:ItemBox', src, DenaliFW.Shared.Items["huntingcarcass2"], "add")

  elseif randomAmount > 23 and randomAmount < 29 then

    Player.Functions.AddItem("huntingcarcass3", 1)
    TriggerClientEvent('inventory:client:ItemBox', src, DenaliFW.Shared.Items["huntingcarcass3"], "add")

  else 

    Player.Functions.AddItem("huntingcarcass4", 1)
    TriggerClientEvent('inventory:client:ItemBox', src, DenaliFW.Shared.Items["huntingcarcass4"], "add")
...
AddEventHandler('remove:money', function(totalCash)
  ...
  if Player.PlayerData.money['cash'] >= (500) then
    ...
    TriggerClientEvent("DenaliFW:Notify", src, 'Reloaded.')
  else
    TriggerClientEvent("DenaliFW:Notify", src, 'Not enough cash on you.', 'error')
  end
end)

DenaliFW.Functions.CreateUseableItem("huntingbait", function(source, item)
