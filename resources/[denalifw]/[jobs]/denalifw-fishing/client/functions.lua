...
TryToFish = function()
    DenaliFW.Functions.TriggerCallback('fishing:GetItemData', function(count)
        if IsPedSwimming(cachedData["ped"]) then
            return DenaliFW.Functions.Notify("You can't be swimming and fishing at the same time.", "error")
        end
        if IsPedInAnyVehicle(cachedData["ped"]) then
            return DenaliFW.Functions.Notify("You need to exit your vehicle to start fishing.", "error")
        end
        if count ~= nil then
            if count == 0 then
                DenaliFW.Functions.Notify("You need both a fishing rod and bait to fish.", "primary")
            else
                ...
                if waterValidated then
                    ...
                    DenaliFW.Functions.Notify("You need to aim towards the water to fish", "primary")
                end
            end
        end
...
CastBait = function(rodHandle, castLocation)
    ...
    if not HasFishingBait() then
        DenaliFW.Functions.Notify('You don\'t have any bait!', 'error')
    ...
    while not IsControlJustPressed(0, 47) do
        ...
        if GetGameTimer() - startedCasting > 5000 then
            DenaliFW.Functions.Notify("You need to cast the bait.", "primary")
    ...
    DenaliFW.Functions.Notify("Waiting for a fish to bite...", "success", "10000")
    TriggerServerEvent('DenaliFW:Server:RemoveItem', "fishingbait", 1)
    TriggerEvent("inventory:client:ItemBox", DenaliFW.Shared.Items["fishingbait"], "remove")
    ...
    if caughtFish then
        ...
        DenaliFW.Functions.Notify("The fish got loose.", "error")
    end
...
IsInWater = function()
    ...
    DenaliFW.Functions.Notify("Checking fishing location...", "success", "3000")
...
function HasFishingBait()
    local rtval = false
    DenaliFW.Functions.TriggerCallback('DenaliFW:HasItem', function(amount)
        if amount then
            rtval = true
        end
    end, "fishingbait")
