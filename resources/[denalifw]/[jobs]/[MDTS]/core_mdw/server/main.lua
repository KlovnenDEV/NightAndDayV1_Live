...
if Config.AccountInformation ~= '' then
            ...
            function(info)
                for _, v in ipairs(info) do
                    ...
                    if OwnedProperties[v.citizenid] == nil then
                        ...
                        OwnedProperties[v.citizenid] = {v.id}
                    else
                        table.insert(OwnedProperties[v.citizenid], v.id)
                    end
                end
            end
        )
            ...
            for _, v in ipairs(info) do
                exports.oxmysql:query(
                    ...
                    function(vehInfo)
                                    ...
                                    local Player = DenaliFW.Functions.GetPlayerByCitizenId(v.citizenid)
                                    ...
                                    if DenaliFW.Functions.GetPlayerByCitizenId(v.citizenid) then
                                        v.online = true
                                    else
                                        v.online = false
                                    end
...
DenaliFW.Functions.CreateCallback(
    "core_mdw:getUsers",
    function(source, cb)
        local user = DenaliFW.Functions.GetPlayer(source).PlayerData.citizenid
        updateUser(user, 0)
        Citizen.Wait(200)
        cb(Users)
    end
    ...
    function(incident)
        ...
        sendToDiscord(
            "Incident removed (#" .. incident .. ")",
            {
                {
                    ["name"] = "Removed By",
                    ["value"] = Users[DenaliFW.Functions.GetPlayer(source).PlayerData.citizenid].name
                }
            }
        )
    end
...
AddEventHandler(
    ...
    function(person, jail, fine, charges)
        local xPerson = DenaliFW.Functions.GetPlayer(person)
...
AddEventHandler(
    ...
    function(person, license)
        ...
        local xPlayer =DenaliFW.Functions.GetPlayerByCitizenId(person)
        local src = source
