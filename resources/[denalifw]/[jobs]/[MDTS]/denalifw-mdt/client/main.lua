...
RegisterNetEvent('DenaliFW:Client:OnGangUpdate', function(GangInfo)
...
RegisterCommand('mdt', function()
    ...
    if not PlayerData.metadata["isdead"] and not PlayerData.metadata["inlaststand"] and not PlayerData.metadata["ishandcuffed"] and not IsPauseMenuActive() then
        ...
        DenaliFW.Functions.Notify("Can't do that!", "error")
    end
end, false)
...
RegisterNetEvent('mdt:client:dashboardWarrants', function()
    DenaliFW.Functions.TriggerCallback("mdt:server:getWarrants", function(data)
        if data then
...
RegisterNUICallback("searchProfiles", function(data, cb)
    ...
    DenaliFW.Functions.TriggerCallback('mdt:server:SearchProfile', function(result)
        p:resolve(result)
    end, data.name)
...
RegisterNUICallback("getProfileData", function(data, cb)
    ...
    local getProfileDataPromise = function(data)
        ...
        p = promise.new()
        DenaliFW.Functions.TriggerCallback('mdt:server:GetProfileData', function(result)
            p:resolve(result)
        end, data)
...
RegisterNUICallback("searchVehicles", function(data, cb)
    ...
    DenaliFW.Functions.TriggerCallback('mdt:server:SearchVehicles', function(result)
        p:resolve(result)
    end, data.name)
...
RegisterNUICallback("saveVehicleInfo", function(data, cb)
    ...
    if JobType == 'police' and impound.impoundChanged == true then
        if impound.impoundActive then
            ...
            if found == 0 then
                DenaliFW.Functions.Notify('Vehicle not found!', 'error')
...
RegisterNUICallback("toggleDuty", function(data, cb)
    ...
    TriggerEvent("denalifw-policejob:ToggleDuty")
    cb(true)
end)
...
RegisterNetEvent('mdt:client:setRadio', function(radio)
    if type(tonumber(radio)) == "number" then
        ...
        DenaliFW.Functions.Notify("You have set your radio frequency to "..radio..".", "success")
    else
        DenaliFW.Functions.Notify("Invalid Station(Please enter a number)", "error")
    end
end)
