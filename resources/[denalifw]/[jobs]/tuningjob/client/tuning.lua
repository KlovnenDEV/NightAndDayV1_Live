...
RegisterNetEvent('6scripts-tuning:client:applyTurbo', function()
    ...
    if IsPedSittingInAnyVehicle(playerPed) then
		DenaliFW.Functions.Notify("Cannot Install while inside vehicle", "error", 3500)
	...
	if IsAnyVehicleNearPoint(coords.x, coords.y, coords.z, 3.5) then
			...
			if Config.isVehicleOwned then
					...
					if owned then
						...
						SetVehicleDoorOpen(vehicle, 4, false, false)
							...
							Skillbar.Start({
								...
								DenaliFW.Functions.Notify("Success! Installing Turbo", "success", 3500)
								FreezeEntityPosition(playerPed, true)
								time = math.random(7000,10000)
								DenaliFW.Functions.Progressbar("", "Installing Turbo", time, false, true, {
								...
								Car = DenaliFW.Functions.GetVehicleProperties(vehicle)
								TriggerServerEvent('6scripts-tuning:server:UpdateVehicle', Car)
								TriggerServerEvent('6scripts-tuning:server:Turbo')
								end)
							end, function()
								DenaliFW.Functions.Notify("Turbo installation failed!", "error", 3500)
								ClearPedTasks(playerPed)
							end)
					else
						DenaliFW.Functions.Notify("Nobody owns this vehicle", "error", 3500)
					end
				end, plate)
			else
				AnimationPlay("mini@repair", "fixing_a_ped", 35000)
				...
				SetVehicleDoorOpen(vehicle, 4, false, false)
					...
					Skillbar.Start({
						...
						FreezeEntityPosition(playerPed, false)
						SetVehicleEngineOn(vehicle, true, true)
						CurrentVehicleData = DenaliFW.Functions.GetVehicleProperties(vehicle)
		...
		DenaliFW.Functions.Notify("There is no vehicle nearby", "error", 3500)
	end
end)
					...
					if owned then
						AnimationPlay("mini@repair", "fixing_a_ped", 35000)
							...
							Skillbar.Start({
								...
								DenaliFW.Functions.Notify("Success! Installing Xenon Headlights", "success", 3500)
								FreezeEntityPosition(playerPed, true)
								time = math.random(3000,7000)
								-- TriggerEvent('pogressBar:drawBar', time, 'Installing Xenon Headlights')
								DenaliFW.Functions.Progressbar("", "Installing Xenon Headlights", time, false, true, {
									disableMovement = false,
									disableCarMovement = false,
									disableMouse = false,
									disableCombat = true,
								...
								DenaliFW.Functions.Notify("Xenon Headlight installation failed!", "error", 3500)
								ClearPedTasks(playerPed)
							end)
...
RegisterNetEvent('6scripts-tuning:client:xenonMenu', function()
	...
	if IsPedSittingInAnyVehicle(playerPed) then
		...
		DenaliFW.Functions.Notify("You need to be inside a vehicle to use this", "error", 3500)
    end
end)
...
RegisterNetEvent('6scripts-tuning:client:SuspensionMenu', function() 
    ...
    if IsPedSittingInAnyVehicle(playerPed) then
		DenaliFW.Functions.Notify("Cannot adjust suspension while inside vehicle", "error", 3500)
        ClearPedTasks(playerPed)
        return
    end
    if IsAnyVehicleNearPoint(coords.x, coords.y, coords.z, 3.0) then
		...
		if DoesEntityExist(vehicle) then
            ...
            DenaliFW.Functions.Notify("You are missing suspension wrenches", "error", 3500)
			end
...
RegisterNetEvent('6scripts-tuning:client:EngineMenu', function() 
	...
	local coords = GetEntityCoords(playerPed)
    if IsPedSittingInAnyVehicle(playerPed) then
		DenaliFW.Functions.Notify("Cannot change engine while inside vehicle", "error", 3500)
        return
    end
    local coords = GetEntityCoords(playerPed)
    local vehicle = GetClosestVehicle(coords.x, coords.y, coords.z, 3.0, 0, 71)
...
RegisterNetEvent('6scripts-tuning:client:InstallEngine', function(args) 
	...
	DenaliFW.Functions.Notify("Success! Installing Engine", "success", 3500)
	time = math.random(7000,10000)
	DenaliFW.Functions.Progressbar("", "Installing Engine", time, false, true, {
	disableMovement = false,
	disableCarMovement = false,
	disableMouse = false,
	disableCombat = true,
    ...
    elseif args == 5 then
		...
		DenaliFW.Functions.Notify("Engine installation failed!", "error", 3500)
		ClearPedTasks(playerPed)
	end)
end)
        ...
        if IsInVehicle and not IsThisModelABike(GetEntityModel(GetVehiclePedIsIn(ped))) then
            if GetPedInVehicleSeat(veh, -1) == ped then
                DenaliFW.Functions.Progressbar("use_nos", "Connecting NOS...", 1000, false, true, {
                    ...
                    TriggerEvent("inventory:client:ItemBox", DenaliFW.Shared.Items['nos'], "remove")
                    TriggerServerEvent("DenaliFW:Server:RemoveItem", 'nos', 1)
