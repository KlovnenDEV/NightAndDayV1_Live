...
function GuyIsComing()
  DenaliFW.Functions.Notify('I am coming!', 'success', 5000)
  player = PlayerPedId()
      ...
      if timer >= 1 then 
        if zrespione == 1 then 
          ...
          if timer <= 1 then
             ...
             timer = 0
             DenaliFW.Functions.Notify('I am going to work', 'success', 5000)
             Citizen.Wait(20000)
...
function GoToTarget(a,b,c,d,e,f)
	TaskVehicleDriveToCoord(e,d, Config.Locations["deliveryPoint"].x,Config.Locations["deliveryPoint"].y,Config.Locations["deliveryPoint"].z,17.0,0,f,drivingStyle,1,true)
	enroute = true
    ...
    if dist < 8 then
		 ...
		 timer = Config.Timer
		 enroute = false
         DenaliFW.Functions.Notify('You have ' ..math.floor(timer)..' seconds, after that i am going to work', 'success', 5000)
		 LastOptions()
		elseif dist < 20 then 
		  Citizen.Wait(2000)
		end 
...
function LastOptions()
	...
	while wybieranie do 
      ...
      DenaliFW.Functions.DrawText3D(a, b, c+1, 'Press ~y~[E]~w~ to see inside ~r~[H]~w~ To Go')
      end
...
function ReadyToGo()
  ...
  TriggerEvent('cd_drawtextui:HideUI')
	...
	SetVehicleDoorShut(towTruck, 3, false)
	timer = 0
  DenaliFW.Functions.Notify('Okay, i hope i was usefull.', 'info', 3500)
	Citizen.Wait(20000)
	DeleteTowTruck(towTruck, towTruckDriver)
end
