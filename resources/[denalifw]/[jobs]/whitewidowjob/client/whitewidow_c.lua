...
exports['denalifw-target']:AddBoxZone("whitewidowduty", Config.WhiteWidowDuty, 1.0, 10.0, {
    name="whitewidowduty",
    heading= 181.63,
    debugPoly=false,
    minZ=53.66,
...
RegisterNetEvent('DenaliFW:Client:SetDuty')
AddEventHandler('DenaliFW:Client:SetDuty', function(duty)
    onDuty = duty
end)
...
exports['denalifw-target']:AddBoxZone("whitewidowstorage", Config.WhiteWidowStorage, 1.0, 40.0, {
...
exports['denalifw-target']:AddBoxZone("whitewidowpay", Config.WhiteWidowPay, 1.0, 40.0, {
    name="whitewidowpay",
    heading= 253.36,
    debugPoly=false,
    minZ=52.664,
...
exports['denalifw-target']:AddBoxZone("whitewidowpay2", Config.WhiteWidowPay2, 1.0, 100.0, {
    name="whitewidowpay2",
    heading= 251.71,
    debugPoly=false,
    minZ=52.664,
...
exports['denalifw-target']:AddBoxZone("whitewidowpay3", Config.WhiteWidowPay3, 1.0, 100.0, {
    name="whitewidowpay3",
    heading= 116.55,
    debugPoly=false,
    minZ=52.664,
...
-- White widow cash register requires denalifw-input
...
exports['denalifw-target']:AddBoxZone("whitewidowshop", Config.WhiteWidowShop, 1.0, 10.0, {
    name="whitewidowshop",
    heading= 347.32,
    debugPoly=false,
    minZ=53.664,
...
exports['denalifw-target']:AddBoxZone("whitewidowtrim", Config.WhiteWidowTrim, 1.0, 10.0, {
    name="whitewidowtrim",
    heading= 165.24,
    debugPoly=false,
    minZ=49.664,
...
exports['denalifw-target']:AddBoxZone("whitewidowjoints", Config.WhiteWidowJoints, 1.0, 10.0, {
...
exports['denalifw-target']:AddBoxZone("whitewidowsnacks", Config.WhiteWidowSnacks, 1.0, 10.0, {
...
exports['denalifw-target']:AddBoxZone("whitewidowtray", Config.WhiteWidowTray, 1.0, 10.0, {
    name="whitewidowtray",
    heading= 161.12,
    debugPoly=false,
    minZ=53.664,
...
exports['denalifw-target']:AddBoxZone("akplants", Config.WhiteWidowWeed4, 1.0, 40.0, {
    name="akplants",
    heading= 68.20,
    debugPoly=false,
    minZ=49.664,
...
RegisterNetEvent('case-whitewidowjob:client:HarvestSkunk', function()
    ...
    if finished ~= 100 then
		DenaliFW.Functions.Notify("Failed to harvest cbd skunk..", "error", 3500)
        ...
        if finished2 ~= 100 then
            ...
            if finished3 ~= 100 then
                ...
                TriggerEvent('pogressBar:drawBar', HarvestTime, 'Harvesting CBD skunk..') 
                Wait(HarvestTime)
...
RegisterNetEvent('case-whitewidowjob:client:HarvestOGKush', function()
	...
	local finished = exports["reload-skillbar"]:taskBar(math.random(5000,7500),math.random(2,4))
    if finished ~= 100 then
		DenaliFW.Functions.Notify("Failed to harvest cbd og-kush..", "error", 3500)
		ClearPedTasks(playerPed)
    else
		local finished2 = exports["reload-skillbar"]:taskBar(math.random(2500,5000),math.random(3,5))
        if finished2 ~= 100 then
			...
			local finished3 = exports["reload-skillbar"]:taskBar(math.random(900,2000),math.random(5,7))
            if finished3 ~= 100 then
				DenaliFW.Functions.Notify("Failed to harvest og-kush..", "error", 3500)
				ClearPedTasks(playerPed)
            else
				local HarvestTime = math.random(15000,20000)
                FreezeEntityPosition(playerPed, true)
                TriggerEvent('pogressBar:drawBar', HarvestTime, 'Harvesting CBD OG Kush..') 
                Wait(HarvestTime)
...
RegisterNetEvent('case-whitewidowjob:client:HarvestWhiteWidow', function()
	...
	local finished = exports["reload-skillbar"]:taskBar(math.random(5000,7500),math.random(2,4))
    if finished ~= 100 then
		DenaliFW.Functions.Notify("Failed to harvest cbd white-widow..", "error", 3500)
		ClearPedTasks(playerPed)
    else
		local finished2 = exports["reload-skillbar"]:taskBar(math.random(2500,5000),math.random(3,5))
        if finished2 ~= 100 then
...
RegisterNetEvent('case-whitewidowjob:client:HarvestAK47', function()
	...
	local finished = exports["reload-skillbar"]:taskBar(math.random(5000,7500),math.random(2,4))
    if finished ~= 100 then
		DenaliFW.Functions.Notify("Failed to harvest cbd ak47..", "error", 3500)
		ClearPedTasks(playerPed)
    else
		local finished2 = exports["reload-skillbar"]:taskBar(math.random(2500,5000),math.random(3,5))
        if finished2 ~= 100 then
...
exports['denalifw-target']:AddBoxZone("whitewidowgarage", Config.WhiteWidowGarage, 1.0, 40.0, {
...
AddEventHandler('case-whitewidowjob:client:SpawnVehicle', function()
	SetNewWaypoint(Config.VehicleSpawn.x, Config.VehicleSpawn.y)
		DenaliFW.Functions.SpawnVehicle(Config.Vehicle, function(veh)
			exports['LegacyFuel']:SetFuel(veh, 100)
			SetVehicleNumberPlateText(veh, 'WWIDOW')
			SetEntityHeading(veh, Config.VehicleSpawnHeading)
			SetEntityAsMissionEntity(veh, true, true)
			TriggerEvent("vehiclekeys:client:SetOwner", GetVehicleNumberPlateText(veh))
			TriggerServerEvent("vehicletuning:server:SaveVehicleProps", DenaliFW.Functions.GetVehicleProperties(veh))
			SetEntityAsMissionEntity(veh, true, true)
			spawnedveh = veh
			rented = true
...
RegisterNetEvent('case-whitewidowjob:client:UseSkunkJoint', function()
	...
	DenaliFW.Functions.TriggerCallback('DenaliFW:HasItem', function(HasItem)
		if HasItem then
				...
				ClearPedTasks(playerPed)
				Wait(1000)
				TriggerServerEvent("DenaliFW:Server:RemoveItem", "weed_skunk_cbd_joint", 1)
				TriggerEvent("inventory:client:ItemBox", DenaliFW.Shared.Items["weed_skunk_cbd_joint"], "remove")
			...
			DenaliFW.Functions.Notify('You need a lighter to smoke..', 'error', 3500)
		end
	end, "lighter")
end)
-- Use joint ogkush
RegisterNetEvent('case-whitewidowjob:client:UseOGKushJoint', function()
	...
	DenaliFW.Functions.TriggerCallback('DenaliFW:HasItem', function(HasItem)
		if HasItem then
				...
				ClearPedTasks(playerPed)
				Wait(1000)
				TriggerServerEvent("DenaliFW:Server:RemoveItem", "weed_og-kush_cbd_joint", 1)
				TriggerEvent("inventory:client:ItemBox", DenaliFW.Shared.Items["weed_og-kush_cbd_joint"], "remove")
...
RegisterNetEvent('case-whitewidowjob:client:UseWhiteWidowJoint', function()
	...
	DenaliFW.Functions.TriggerCallback('DenaliFW:HasItem', function(HasItem)
		if HasItem then
				...
				ClearPedTasks(playerPed)
				Wait(1000)
				TriggerServerEvent("DenaliFW:Server:RemoveItem", "weed_white-widow_cbd_joint", 1)
				TriggerEvent("inventory:client:ItemBox", DenaliFW.Shared.Items["weed_white-widow_cbd_joint"], "remove")
...
RegisterNetEvent('case-whitewidowjob:client:UseAK47Joint', function()
	...
	DenaliFW.Functions.TriggerCallback('DenaliFW:HasItem', function(HasItem)
		if HasItem then
				...
				ClearPedTasks(playerPed)
				Wait(1000)
				TriggerServerEvent("DenaliFW:Server:RemoveItem", "weed_ak47_cbd_joint", 1)
				TriggerEvent("inventory:client:ItemBox", DenaliFW.Shared.Items["weed_ak47_cbd_joint"], "remove")
...
-- Function to close denalifw-menu
