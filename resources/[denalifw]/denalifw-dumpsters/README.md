# mrp-dumpster
This is a rework by Mickey#4910 (Mickeystix/Marcus) for use with BerkieBb's denalifw-target.

Using denalifw-target, you can initiate dumpster diving. If the dumpster is valid (meaning it has not been searched) then you get random items (see config.lua for item configuration, you can change amounts in server.lua). After a dumpster is searched, it is unavailable for 1-60 minutes (determined randomly, but can change in server.lua)

To integrate with denalifw-target, add this to the init.lua (sometimes called the "config" method of adding new interactions)
...
denalifw-target is required - https://github.com/BerkieBb/denalifw-target
