...
AddEventHandler('qb-dumpster:server:giveDumpsterReward', function()
    ...
    if Player.Functions.AddItem(randomItem, 1) then
        TriggerClientEvent("inventory:client:ItemBox", source, DenaliFW.Shared.Items[randomItem], "add")
    else
        TriggerClientEvent("DenaliFW:Notify", source, "You dont have enough space", "error")
    end
end)
