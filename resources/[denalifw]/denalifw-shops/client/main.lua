...
RegisterNetEvent('denalifw-shops:marketshop', function()
                ...
                DenaliFW.Functions.TriggerCallback("denalifw-shops:server:getLicenseStatus", function(hasLicense, hasLicenseItem)
                    ...
                    if Config.Locations[shop].products == Config.Products["weapons"] then
                        if hasLicense and hasLicenseItem then
                            ...
                            DenaliFW.Functions.Notify("success.dealer_verify")
                            Wait(500)
                        else
                            for i = 1, #products do
                                if not products[i].requiredJob then
                            ...
                            DenaliFW.Functions.Notify("error.dealer_decline")
                            Wait(500)
                            DenaliFW.Functions.Notify("error.talk_cop")
                            Wait(1000)
                        end
                    else
                        ShopItems.items = SetupItems(shop)
