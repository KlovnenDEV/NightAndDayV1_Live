...
local DenaliFW, ESX, PlayerLoaded, PlayerData
...
if Config.Framework == 'DenaliFW' then
	Citizen.CreateThread(function()
		while DenaliFW == nil do
			TriggerEvent(Config.MainCoreEvent, function(obj) DenaliFW = obj end)
			if DenaliFW == nil then
				DenaliFW = exports[Config.CoreResourceName]:GetCoreObject()
			end
			Citizen.Wait(100)
		end
...
function Functions.EnableTarget()
	if not AllowTarget or success or (Config.Framework == 'DenaliFW' and not isLoggedIn) or (Config.Framework == 'ESX' and not PlayerLoaded) then return end
...
TriggerEvent('chat:client:ClearChat', source)
										else
    ...
    CreateThread(function()
        ...
        elseif data.event then
            ...
            elseif data.type == "qbcommand" then
                TriggerServerEvent('DenaliFW:CallCommand', data.event, data)
            else
                TriggerEvent(data.event, data)
            end
        else
