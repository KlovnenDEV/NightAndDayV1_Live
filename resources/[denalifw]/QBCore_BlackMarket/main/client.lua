    ...
    while true do 
        ...
        for i = 1, #Config.BlackMarket, 1 do
            ...
            if DistanceBlackMarket < 3.0 then
                Sleep = 2
                DenaliFW.Functions.DrawText3D(Config.BlackMarket[i].NPC.x, Config.BlackMarket[i].NPC.y, Config.BlackMarket[i].NPC.z + 2.0, '~r~E~w~ - '..Config.BlackMarket[i].Satici)
                if IsControlJustPressed(0, 38) then
                    DenaliFW.Functions.TriggerCallback('server:BlackMarket:server:PoliceSayi', function(count)
                        if count >= Config.BlackMarket[i].Police then
                            OpenBlackMarket()
                        else
                            DenaliFW.Functions.Notify("Not Now!","There aren't enough cops in the city!",3000,"error")
                        end
                    end)
                end
            end
